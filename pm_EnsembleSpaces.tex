...
\begin{defn}
	The \textbf{scalar multiplication} of a difference $s(\ens[b] - \ens[a])$ by a real number $r \in \mathbb{R}$ is the difference $(rs)(\ens[b] - \ens[a])$
\end{defn}
...
\begin{defn}
	Given an internal point $\ens[a]$, the map $\ens[b] \to (\ens[b] - \ens[a])$ is the \textbf{natural embedding} of the vector space in the space of differences from $\ens[a]$.
\end{defn}

\begin{prop}
	Let $\ens[a],\ens[b] \in \Ens$ be two internal points. Then $r(\ens[c] - \ens[b]) \mapsto r(\ens[c]-\ens[a]) - r(\ens[b]-\ens[a])$ is an isomorphism between the corresponding vector spaces. The ensemble space is therefore endowed with an affine structure.
\end{prop}
