...
\begin{conj}
	Let $\Ens$ be an ensemble space. The set of boundary points $B_{\Ens}$ is not necessarily a closed set and therefore $I_{\Ens}$ is an open set.
\end{conj}
\begin{proof}
...
\titledbreak{Open problems and old attempts}

There are still several open problems, with half-finished or failed attempts. Here we collect some of the ideas and trials, which likely include wrong results. They should mainly serve as inspiration for future attempts.

\begin{openproblem}{Affine sets and affine hulls}{pm-es-affineHullProblem}{Ensemble spaces, Convex spaces, Affine spaces}
...
Another open question is the right characterization of classical spaces. Part of the problem is isolating the different parts that may lead to different features that one may consider classical.

For example, finite discrete classical spaces are simplexes such that all extreme points are orthogonal to each other and have zero entropy. One may have a simplex where all extreme points are orthogonal, though the extreme points have different entropy. All features of a classical theory are there, expect that the entropy function has to be corrected to $S(\ens) = I(p_i) + \sum_i p_i S(x_i)$. A weaker case is when the ensemble space may be a simplex, but the extreme points are not orthogonal.
...
Classical contexts represent subsets of ensemble spaces that can be understood as classical regions. There is again the difference of just considering the convex structure (i.e. they form a simplex) or also add the entropic structure (i.e. separate ensembles are orthogonal). In the attempt below, we chose to only use the convex structure. These are still rough ideas that may need to be refined.
...
We still need to understand what requirements a measure must satisfy to describe a well-posed physical problem. The issue is that the set of all probability measures defined over a space is too broad, but it is not clear how it should be restricted.
...
The definition of the spectrum of a quantity (i.e. the allowed values) is still an open problem. We leave here the latest attempt, more for inspiration and reference.
...
\section{Alternative definition for contexts}
