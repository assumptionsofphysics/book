...
\begin{proof}
	Let $n=\dim(\Ens)$. Since $\ens[a]$ is algebraically internal in $A$, we can find ensembles $\ens_1,\dots,\ens_n\in A$ such that the directions from $\ens[a]$ to $\ens_i$ form an affine basis. Since $\ens[a]$ is algebraically internal in $A$, for each $\ens_i$ there exists $\ens_i'\in A$ and $p_i\in(0,1)$ such that
	\begin{equation}
		\ens[a]=p_i\ens_i+\bar p_i\ens_i'.
	\end{equation}
	Thus, for each basis direction, $A$ contains points on both sides of $\ens[a]$.
	
	Choose sufficiently small mixtures of these points with $\ens[a]$ so that the resulting $n+1$ points form a simplex $\Delta\subseteq A$ of full affine dimension with $\ens[a]\in\Delta\setminus B_\Delta$. Since $A$ is convex and all vertices of $\Delta$ lie in $A$, we have $\Delta\subseteq A$.
	
	By the previous proposition, $\ens[a]$ is a topological interior point of $\Delta$. Therefore there exists an open neighborhood $U$ of $\ens[a]$ such that
	\begin{equation}
		U\subseteq\Delta.
	\end{equation}
	Since $\Delta\subseteq A$, we have $U\subseteq A$. Therefore $\ens[a]$ is a topological interior point of $A$.
\end{proof}
