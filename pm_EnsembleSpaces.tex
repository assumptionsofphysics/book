...
\begin{justification}
	...
	The variability increase is maximal when the two ensembles are completely distinct. In this case, the increase in variability is given only by the coefficients and must follow the Shannon entropy as it is the only continuous indicator of variability that is linear in probability. This justifies the upper bound. If an ensemble is completely distinct from another, it will also be distinct from all its components. Converse, if an ensemble is completely distinct from two others, it will be completely distinct from all mixtures. That is, distinctness is preserved by mixing, which justifies the last property of the entropy.
\end{justification}
...
\begin{conj}
	Let $\ens_1, \ens_2, \ens_3 \in \Ens$ such that $\ens_1$ and $\ens_2$ are distinct and $\ens_1$ and $\ens_3$ are distinct. Then $\ens_1$ is distinct from all the mixtures of $\ens_2$ and $\ens_3$.
\end{conj}
...
\begin{conj}
	Let $\ens,\ens_1,\ens_2 \in \Ens$. Then $\ens$ does not have a common component with a mixture of $\ens_1$ and $\ens_2$ if and only if $\ens_1$ does not have a common component with either $\ens_1$ and $\ens_2$.
\end{conj}
