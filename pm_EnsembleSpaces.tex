...
\begin{axiom}[Axiom of entropy]
	An ensemble space $\Ens$ is equipped a function $S : \Ens \to \mathbb{R}$, defined up to a multiplicative constant, called \textbf{entropy} with the following properties
	\begin{itemize}
...
\begin{prop}\label{pm_es_entropyUnique}
...
\begin{proof}
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	The equality hold if and only if $\frac{p\rho_1 + \bar{p}\rho_2}{\rho_1} = 1$ which is exactly when $\rho_1 = \rho_2$.
	
	For the upper bound, note that the logarithm is a strictly increasing function, and therefore $- \log(x + k) \leq -\log(x)$ for any $k \geq x$, with equality holding if and only if $k=0$. We have
	\begin{equation}
		\begin{aligned}
			S(p\rho_1 + \bar{p}\rho_2) &= - \int_X \left(p\rho_1 + \bar{p}\rho_2\right) \log \left(p\rho_1 + \bar{p}\rho_2\right) d\mu \\
			&= - \int_X p\rho_1 \log \left(p\rho_1 + \bar{p}\rho_2\right) d\mu - \int_X \bar{p}\rho_2 \log \left(p\rho_1 + \bar{p}\rho_2\right) d\mu \\
		...
		\end{aligned}
	\end{equation}
	The equality to hold, if and only if $\rho_2=0$ wherever $\rho_1\neq0$ and vice-versa. That is, the equality holds if and only if the two distributions have disjoint support. Therefore orthogonal distributions are exactly distributions with disjoint support.
	
	Note that two distributions that have no common components. That is, there is no $\rho_3$ such that $\rho_1 = p \rho_3 + \bar{p} \rho_4$ and $\rho_2 = q \rho_3 + \bar{q} \rho_5$. Therefore orthogonality implies disjunctness which implies separatedness.
	...
	All these arguments are valid for discrete classical ensemble spaces, changing integrals with sums.
...
\subsection{Entropic constraints on the space}
...
\begin{proof}
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	which is true since $p_0 \in (0,1)$. Therefore, since we can always find a greater $p$ for which the relationship holds, combined with the previous result, $p\ens[a] + \bar{p} \ens = p \ens[b] + \bar{p} \ens$ for all $p \in (0,1)$.
