...
\begin{axiom}[Axiom of mixture]
	...
	\begin{itemize}
		...
		\item \textbf{Associativity}: $p_1 \ens_1 + \bar{p}_1\left(\frac{1-p_1-p_3}{\bar{p}_1}\ens_2 + \frac{p_3}{\bar{p}_1}\ens_3\right) =  \bar{p}_3\left(\frac{p_1}{\bar{p}_3} \ens_1 +  \frac{1-p_1-p_3}{\bar{p}_3}\ens_2\right) + p_3 \ens_3$ where $p_1 + p_3 \leq 1$
	\end{itemize}
\end{axiom}
...
\begin{defn}[Finite mixture]
	...
	$$p_1 \ens_1 + (1-p_1)\left( \tfrac{p_2}{1-p_1} \ens_2 + \tfrac{1-p_1-p_2}{1-p_1} \left( \tfrac{p_3}{1-p_1-p_2} \ens_3 + \tfrac{1-p_1-p_2-p_3}{1-p_1-p_2}\left(\ldots+\left(\tfrac{p_{n-1}}{1- p_1 + \ldots + p_{n-1}} \ens_{n-1} + \tfrac{p_n}{1- p_1 + \ldots + p_{n-1}}\ens_n\right)\right) \right) \right).$$
\end{defn}
...
\begin{remark}
	Note that we can collect and expand convex combination into other convex combinations. Because coefficients always sum to one, we can always calculate the new coefficients from one side. For example, $p_a \ens[a] + p_b \ens[b] + p_c \ens[c] + p_d \ens[d] = p(\frac{p_a}{p} \ens[a] + \frac{p_b}{p} \ens[b]) + \bar{p}(\frac{p_c}{\bar{p}} \ens[c] + \frac{p_d}{\bar{p}} \ens[d])$ where $p = p_a + p_b$ is defined only on the left part. Since $1 = p + \bar{p} = p_a +p_b + p_c + p_d$, $\bar{p} = p_c + p_d$.
\end{remark}

The topology also allows us to extend the notion to infinite mixtures. Not all infinite mixtures will be allows, and it is the topology that will tell us which series are going to converge or not.
...
\begin{conj}[Convergence of submixtures]
...
\begin{mathSection}
	...
	\begin{thrm}[Differences from a vector space]
...
\begin{prop}[Open sets are not enough]\label{pm_es_openSetsNotEnough}
