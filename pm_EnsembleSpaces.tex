...
\begin{defn}
	Let $\ens \in \Ens$ be an ensemble and $A \subseteq \Ens$ a set of ensembles. Then the \textbf{$A$-components of $\ens$} are the components of $\ens$ that are mixtures of $U$. That is, $A_{\ens} = \{ \ens_1 \in \hull(U) \, | \, \exists p \in (0,1], \ens_2 \in \Ens \text{ s.t. } \ens = p \ens_1 + \bar{p} \ens_2  \}$. An \textbf{increasing $A$-component sequence of $\ens$} is an increasing component sequence of $\ens$ such that $\{\ens[a]_i\} \subseteq A_{\ens}$. The sequence is \textbf{maximal component sequence} $\{\ens[x]_i\} \subseteq \hull(A)$ if the limit $p_i \to p$ is the supremum of all such sequences.
\end{defn}
...
\begin{coro}
	Let $\ens_1, \ens_2 \in \Ens$. Then $p_{\ens_1} \neq p_{\ens_2}$.
\end{coro}
...
\begin{coro}
	Let $\{\ens[a]_i\} \subseteq A_{\ens}$ be a maximal sequence and $\{p_i\} \subseteq (0,1]$ its associated coefficients. Then $p_i \to p_{\ens}(U)$.
\end{coro}
