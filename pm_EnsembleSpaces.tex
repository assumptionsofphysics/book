...
\openproblem{Spectrum of a quantity}{pm-es-quantitySpectrum}{Ensemble spaces}{Spectral theory, Topological vector spaces, Order theory}{}{We are looking for a definition of the spectrum of a quantity (i.e. linear functional) that can be defined for a general ensemble space (i.e. convex space). It would already be helpful to establish whether the entropic structure is required, or whether the convex structure is sufficient.}

The main open issue is to understand whether the spectrum \textbf{requires} a notion of entropy or not. In both classical and quantum mechanics, given a quantity $f$, every Borel set $A \subseteq \mathbb{R}$ of $f$ corresponds to a subspace of ensembles that is orthogonal to the subspace corresponding to $A^{\complement}$. For example, in classical mechanics it would correspond to all measures supported by $A$ and in quantum mechanics all density operators for which the eigenspace for $1$ is a subspace of $A$. We could therefore start by taking sets like $(-\infty, a)$ and $(-\infty, a]$ and find the biggest orthogonal subspace that contains only ensembles with expectation $<a$ or $\leq a$. Subspaces for all other Borel sets can be recovered closing the lattice. For each subspace $A$ in the lattice, we can then define a statistical variable that is one for all elements of $A$ and zero for all elements of $A^{\ortho}$. This recovers the probability. This type of definition would recover the notion in classical and quantum mechanics, but would only work on orthogonally decomposable ensemble spaces.

Below is outlined an idea for extending the notion without requiring the lattice of orthogonal subspaces. The idea is that orthogonal subspaces are affine sets and include all the components. Maybe those properties are enough to define the lattice of supported ensembles.

Let us first define a component set as one that includes all the components of its elements. We can then create a closure operation that, given a set, returns the set of all its components. This, again, gives us an intersection structure.
...
It may be possible if we use the lattice of affine sets (called flats in the definitions below). 
