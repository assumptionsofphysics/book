...
The fact that the ensemble space is directionally bounded gives us an intuitive property that we would, mistakenly, always think to be true. Suppose we have an ensemble $\ens$ and a sequence of ensembles $\ens[a]_i \in L$ on some line $L$ that contains $\ens$. Then $p_i \ens + \bar{p}_i \ens[a]_i \to \ens$ if $p_i \to 1$. This does not work in a generic convex space if directions are unbounded. For example, suppose $\ens = \mathbb{R}$, which is a convex set but not directionally bounded. Let $\ens = 0$, $\ens[a]_i = i$ and $p_i = 1 - \frac{1}{i}$. Then $\left(1 - \frac{1}{i}\right) 0 + \frac{1}{i}{i} = 1$. Therefore $p_i \ens + \bar{p}_i \ens[a]_i \nto \ens$ even though $p_i \to 1$. It is, again, the entropy that guarantees that this intuitive property is satisfied.
...
It is an open question whether this property is guaranteed for a generic sequence of ensembles $\ens[a]_i$, not necessarily on a line. At least, it should be extensible to the finite-dimensional case.

\begin{conj}
	Let $\ens \in \Ens$ and $\ens[a]_i \in \Ens$. Then $p_i \ens + \bar{p}_i \ens[a]_i \to \ens$ if $p_i \to 1$.
\end{conj}
...
\begin{conj}\label{pm-es-conjSubmixturesConverge}
...
\begin{conj}\label{pm-es-conjAlgebraicInteriorSigmaConvex}
