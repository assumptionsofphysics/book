...
\begin{figure}[h]
	\includegraphics[width=0.5\textwidth]{tempimages/MultipleDecomposition.jpg}
\end{figure}
...
\begin{defn}
	Let $\Ens$ be an ensemble space. A \textbf{classical probability context} is a flat $C \subset \Ens$ where each decomposable element in $C$ is separately decomposable in $C$ but not separately multidecomposable $C$.
\end{defn}
...
\begin{proof}
	First we show that $C$ contains only three types of ensembles: those that are limits of convex mixtures of only elements $A$, those that are limits of convex mixtures of $B$, those that are limits of convex mixtures of both $A$ and $B$. For an ensemble to not be in both $A$ and $B$, it would be separate from both which, by construction, is not possible: if it separate from $A$ it would be separate from $U$ and therefore would be in $B$. Since separate multidecomposition is forbidden, an ensemble $\ens$ cannot be written both as a convex combination of $A$ and as a convex combination with an element of $B$. This would yield two decompositions in which one components, the one chose from $B$, is separate from the all the components of the other. This means that an element of $C$ is either a mixture of $A$, a mixture of $B$ or a mixture of both $A$ and $B$.
	...
	To see that $C$ is the closed convex closure of $A$ and $B$, not that all the elements in $C$ that are not already in $A$ and $B$ are the mixtures of $A$ and $B$. These are exactly added when performing the closure.
...
\begin{figure}[h]
	\includegraphics[width=0.5\textwidth]{tempimages/CounterexampleProbabilityContexts.jpg}
\end{figure}
...
\begin{conj}
	The property of classical probability context is exactly what allows $A$ and $B$ to be convex sets.
\end{conj}

\begin{conj}
	Let $C \subseteq \Ens$ be a classical probability context. Let $\mathfrak{L}$ be lattice of $\separate$-subspaces. Then the fraction is a linear set function on the lattice. That is, for all $\ens \in C$ and $A, B \in \mathfrak{L}$ such that $A \cap B = \emptyset$,  $\fcap_{\ens}(A \cup B) = \fcap_{\ens}(A) + \fcap_{\ens}(B)$.
\end{conj}
