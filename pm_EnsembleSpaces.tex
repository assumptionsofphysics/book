...
Since ensemble spaces embed in vector spaces, it is useful to define affine sets and affine hulls as closures over not only convex combinations but affine combinations. We can then relate these construction to the intersection of subspaces of the embedding vector spaces with the ensemble spaces. This allows us to define affine basis and affine dimensionality of the ensemble space in a way that they map to the usual notions of basis and dimensionality in vector spaces.

A set is affine if it allows all possible finite affine combinations. To extend this to the infinite case we use the topology. A set is closed and affine if it includes all limits of finite affine combinations. Note that we have not yet defined infinite affine combination, which is left as an open problem.
...
\begin{mathSection}
	...
	\begin{remark}
		The above definition does not need infinite affine combinations to be defined because it just uses topological closure. Since affine combinations, unlike convex combination, go ``in both directions'', it is not useful to define an affine analogue of $\sigma$-convex sets as they would coincide with closed and affine sets.
	\end{remark}
\end{mathSection}

Given a set of ensembles $A$, we can ask for all ensembles that can be constructed from $A$. The affine hull of $A$ is the set of all finite affine combinations of $A$ while the closed affine hull of $A$ is the set of all topological limits of finite affine combinations of $A$.
...
Having defined affine sets, we can define a notion of affine independence. In intuitively, for a set linearly independent vectors, one vector cannot be expressed as a linear combination of the other. We, therefore, define an affinely independent set of ensembles as one in which every ensemble cannot be expressed as an affine combinations of the other. We can then define a notion of affine basis as a set of ensembles whose affine hull is the whole space.

\begin{mathSection}
\begin{defn}
	A set $A \subseteq \Ens$ of ensembles is said to be \textbf{affinely independent} provided that $\forall \ens[a]\in A, \ens[a]\notin\ahull(A\backslash\{\ens[a]\})$. An \textbf{affine basis} $\mathcal{B} \subseteq \Ens$ of ensembles is an affinely independent set of ensembles whose affine hull $\ahull(\mathcal{B}) = \Ens$ is the whole space.
\end{defn}

\begin{remark}
	We use the affine hull to match the standard definitions in a vector space. It may be, however, that using closed affine hull may make more physical sense.
\end{remark}

TODO: hulls are closure operations

\begin{coro}
	Let $\ens[a] \in \Ens$ be an internal point. If $\mathcal{B} \subseteq \Ens$ is a basis, then we can find $\ens \in \mathcal{B}$ such that $\{\ens[a]\} \cup \mathcal{B} \setminus \{\ens\}$ is a basis. That is, every basis can be centered at any internal point.
\end{coro}

\begin{proof}
	Since $\mathcal{B}$ is a basis, $\ens[a] = \sum_{i=1}^n r_i \ens[b]_i$ for some $n$, $\{r_i\} \in (0,1]$ and $\{\ens[b]_i\} \in \mathcal{B}$. Consider  $\mathcal{B}' = \{\ens[a]\} \cup \mathcal{B} \setminus \{\ens[b]_1\}$. Since $\ens[b_1] = \frac{1}{r_1} \left(\ens[a] - \sum_{i=2}^n r_i \ens[b]_i\right)$, $\ens[b]_1 \in \ahull(\mathcal{B}')$ and therefore $\ahull(\mathcal{B}') = \ahull(\mathcal{B}' \cup \{\ens[b]_1\}) \supseteq \ahull(\mathcal{B}) = \Ens$. Which means $\ahull(\mathcal{B}') = \Ens$.
	
	Note that, since $\mathcal{B} \setminus \{\ens[b]_1\} = \mathcal{B}' \setminus \{\ens[a]\} $, we have $\ahull( \mathcal{B} \setminus \{\ens[b]_1\} ) = \ahull( \mathcal{B}' \setminus \{\ens[a]\} )$, which means that $\ens[a] \notin \ahull( \mathcal{B}' \setminus \{\ens[a]\} )$. Now, suppose $\ens[b] \in \ahull( \mathcal{B}' \setminus \{\ens[b]\} )$ for some $\ens[b] \in \mathcal{B}'$, which means $\ens[b] \neq \ens[a]$. Then we can write $\ens[b] = r_a \ens[a] + \sum_{i=1}^n r_i \ens[b]_i$ for some $n$, $\{r_i\} \in (0,1]$ and $\{\ens[b]_i\} \in \mathcal{B}' \cup \mathcal{B} \setminus \{ \ens[b]\}$. Since $\ens[a]\neq\ens[b]$, $r_a \in [0,1)$. If $r_a$ is zero, then we would have $\ens[b] \in \ahull (\mathcal{B} \setminus \{\ens[b]\})$, which can't be.  If $r_a \neq 0$, since $r_a \neq 1$ we would be able to re-express $\ens[b]$ as an affine combinations of elements of $(\mathcal{B} \setminus \{\ens[b]\})$. But this, again, is not possible, therefore  $\ens[b] \in \ahull( \mathcal{B}' \setminus \{\ens[b]\} )$ for all $b \notin \mathcal{B}'$. Which means $\mathcal{B}'$ is a basis.
\end{proof}
\end{mathSection}
...
\begin{mathSection}
\begin{prop}
	Let $A \subseteq \Ens$ be a set of ensembles containing an internal ensemble $\ens[a] \in \Ens$. Then the affine hull of $A$ is exactly the intersection between the span of $\iota_{\ens[a]}(A)$ and the ensemble space. That is, $\iota_{\ens[a]}(\ahull(A)) = \vspan(\iota_{\ens[a]}(A \setminus \{\ens[a]\})) \cap \iota_{\ens[a]}(\Ens)$.
\end{prop}
\begin{proof}
...
\begin{proof}
	...
	If $\ahull(A) = \Ens$, then $\iota_{\ens[a]}(\ahull(A)) = \iota_{\ens[a]}(\Ens)$. Now consider $v = [r(\ens[b] - \ens[a])] \in V_{\ens[a]} \setminus \iota_{\ens[a]}(\Ens)$. Note that $r \neq 0$, since the origin corresponds to $0 = \iota_{\ens[a]}(\ens[a]) \in \iota_{\ens[a]}(\Ens)$. Then $ v = r [1(\ens[b] - \ens[a])] = r \iota_{\ens[a]}(\ens[b]) \in \iota_{\ens[a]}(\Ens)$. Therefore every element that is not in $\iota_{\ens[a]}(\Ens)$ is in its span. This means $\vspan(\iota_{\ens[a]}(A)\setminus\{\ens[a]\}) = V_{\ens[a]}$.
\end{proof}
\end{mathSection}

We are now in the position to show that the notion of affine basis correspond to the notion of vector space basis. This allows us to easily show that affine basis always exist and that a notion of affine dimension for the ensemble space can be defined.
