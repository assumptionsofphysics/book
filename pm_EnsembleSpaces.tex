...
\begin{axiom}[Axiom of entropy]
	An ensemble space $\Ens$ is equipped with a function $S : \Ens \to \mathbb{R}$, defined up to a multiplicative constant, called \textbf{entropy} with the following properties
	\begin{itemize}
		\item \textbf{Continuity}\footnote{Currently, we are imposing that the entropy is continuous. There may a chance that this requirement is redundant, as strict concavity and the upper variability bound may already impose this. We have found proofs that show that real valued convex/concave functions of real values are continuous. These proofs fail at the boundary, but the upper variability bound may fix this. Another open question is whether differentiability is also an independent requirement.}
...
\begin{thrm}[Uniqueness of entropy]\label{pm_es_entropyUnique}
	The entropy of the coefficients $I(p,\bar{p})$ is the Shannon entropy. That is, $I(p,\bar{p}) = - k \left(p \log p - \bar{p} \log \bar{p}\right)$ where $k$ is the arbitrary multiplicative constant for the entropy. For a mixture of arbitrary many elements, $I(\{p_i\}) = - k \sum p_i \log p_i$.
\end{thrm}
...
\begin{proof}
	...
	\begin{equation}
		\begin{aligned}
			...
			&=  - \int_X p\rho_1 \log p d\mu - \int_X p\rho_1 \log \rho_1 d\mu - \int_X \bar{p}\rho_2 \log \bar{p} - \int_X \bar{p}\rho_2 \log \rho_2 d\mu\\
	...
	Note that two distributions that have disjunct support have no common components. That is, there is no $\rho_3$ such that $\rho_1 = p \rho_3 + \bar{p} \rho_4$ and $\rho_2 = q \rho_3 + \bar{q} \rho_5$. This is because $\rho_3$ would have to have support that is both within the support of $\rho_1$ and $\rho_2$. Therefore orthogonality implies separatedness.
...
The axioms of entropy not only constrain the functional form of the entropy, but they also constrain the type of space we can have. In particular, it rules out some spaces that would seem physically pathological.
...
\begin{prop}
	Let $\ens[a], \ens[b], \ens \in \Ens$ such that $p\ens[a] + \bar{p} \ens = p \ens[b] + \bar{p} \ens$ for some $p \in (0,1)$. Then $\ens[a] = \ens[b]$.
\end{prop}

\begin{proof}
	Let $\ens[a], \ens[b], \ens \in \Ens$ such that $p_0\ens[a] + \bar{p}_0 \ens = p_0 \ens[b] + \bar{p}_0 \ens$ for some $p_0 \in (0,1)$.
	
	First, we show that the relationship holds for all $p \in (0,p_0]$. In that case, since $0 < \frac{p}{p_0} \leq 1$, we have
	\begin{equation}
		\begin{aligned}
	...
	Now that we show the relationship holds for all $p \in (0,1)$. Since we want to be able to expand multiple times, we want to be able to find a $p$ such that
	\begin{equation}
		p \ens[a] + p \ens[a] + (1-2p) \ens = p \ens[a] + \bar{p} (p_0 \ens[a] + \bar{p}_0 \ens).
	\end{equation}
	That is, we want $\bar{p} p_0 = p$, which means
	\begin{equation}
		\begin{aligned}
			p&=(1-p)p_0=p_0 - p p_0 \\
			(1+p_0) p &= p_0 \\
	...
	\begin{equation}
		\begin{aligned}
			2p \ens[a] + \bar{2p} \ens &=  p \ens[a] + p \ens[a] + (1-2p) \ens =  p \ens[a] + \bar{p} (p_0 \ens[a] + \bar{p}_0 \ens) = p \ens[a] + \bar{p} (p_0 \ens[b] + \bar{p}_0 \ens) \\
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	which is true since $p_0 \in (0,1)$. Therefore, since we can always find a greater mixing coefficient greater the relationship holds, combined with the previous result, $p\ens[a] + \bar{p} \ens = p \ens[b] + \bar{p} \ens$ for all $p \in (0,1)$.
	...
	Lastly, we show that $S(\lambda \ens[a] + \bar{\lambda} \ens[b]) = S(a) = S(b)$ for all $\lambda \in (0,1)$. Using the entropy bounds, with $\lambda \in [0,1]$, we have:
	\begin{equation}
		\begin{aligned}
	...
	\begin{equation}
		\begin{aligned}
			...
			S(0\ens[a] + 1\ens[b]) &= S(\ens[b]) = S(\ens[a]) \\
		\end{aligned}
	\end{equation}
	Since $S(\lambda\ens[a] + \bar{\lambda}\ens[b]) = \lambda S(\ens[a]) + \bar{\lambda} S(\ens[b])$, $\ens[a] = \ens[b]$.
\end{proof}
