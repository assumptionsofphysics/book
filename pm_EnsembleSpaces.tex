...
\begin{prop}
	Let $\Ens$ be a discrete or classical ensemble space. Then $\ens[a] \separate \ens[b]$ if and only if they correspond to measures with disjoint support.
\end{prop}

\begin{proof}
	Note that the support of a convex combination of probability measures is the union of the support of the measures. Since we are restricting ourselves to probability measures that are absolutely continuous with respect with a measure $\mu$, if $p$ has support $U$, any measure $\lambda$ with support on a compact subset $V \subseteq U$ such that $\mu(V) \neq 0$ is a component of $p$. 
	
	Let $\ens[a], \ens[b] \in \Ens$ be two probability measures over the corresponding $X$ absolutely continuous with the corresponding $\mu$. Suppose $\ens[a]$ and $\ens[b]$ have overlapping support. The we can find a compact subset $U$ such that $\mu(U) \neq 0$ and is a subset of the intersection of their supports. Therefore, we can find a probability measure that is a component of both. Conversely, suppose they have disjoint support. Then they cannot have a common component, as the support of the common component would have to a non-empty subset of both supports.
\end{proof}
...
\begin{proof}
	If $\ens \in \Ens$ is decomposable, then it has at least to distinct components, one of which must not be $\ens$. Since an extreme point as only itself as a component, then $\ens$ is not an extreme point. Conversely, if $\ens$ is an extreme point, it cannot be the mixture of two distinct components as $\ens$ has only itself as a component.
\end{proof}

\begin{defn}
	An ensemble space is \textbf{separately decomposable} if every decomposable ensemble is separately decomposable, \textbf{multidecomposable} if every decomposable ensemble is multidecomposable, and \textbf{monodecomposable} if every decomposable ensemble is monodecomposable and \textbf{separately monodecomposable} if it is both separately decomposable and monodecomposable.
\end{defn}
...
\begin{proof}
	For a classical ensemble space, only Dirac measure are not decomposable. For a discrete classical space, every measure that is not the Dirac measure is the mixture of two measures with disjoint support. For a continuous classical space, there are no extreme points, and any absolutely continuous measure is the mixture of two absolutely continuous measures with disjoint support. Since measure with disjoint support are separate, every decomposable ensemble is separately decomposable.
...
\subsubsection{Flats and affine combinations}
...
It still needs to be understood what the best definition for flat is, their relationship with the topological closure, define a corresponding hull and so on. The following definition represent an early attempt that needs to be revised and finalized.
...
Another open question is the right characterization of classical spaces and classical contexts within a non-classical theory. Part of the problem is isolating the different parts that may lead to different features that one may consider classical.
...
We will also want to define a classical context within a non-classical theory, to handle recovering classical probability for measurement outcomes. The basic idea is that a set of orthogonal states will form a simplex, so we can understand projection measurement as moving the system to one of those states. Ideally, a classical context should be a subset of an ensemble space that is a classical ensemble space. Again, one may want to differentiate the three cases: just a simplex, a simplex with orthogonal extreme points, or a simplex with orthogonal extreme points with the same entropy. An additional problem is the multiplicity of extreme points for the same measurement outcome. In that case, the set of post-measurement ensembles are not a simplex. It may be that the correct definition for a classical context would be the domain of a map to simplex that preserves orthogonality.

The general idea is that classical spaces allow ensembles to be decomposed into mutually exclusive, orthogonal, ensembles. If two ensembles have no common component, it is because they have no elements in common and are therefore mutually exclusive.
...
\subsubsection{Sets decomposability}
...
\subsubsection{Probability measures from classical probability contexts}
...
\subsection{Poisson (or Lie) bracket}

\subsection{Recovering limiting pure states}
