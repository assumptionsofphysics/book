...
\begin{mathSection}
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}
			...
			% The ensemble space
			\draw[-, thick] plot [smooth cycle] coordinates {(0.5,1.5)(2,0.2)(5,0.5)(6,2)(5.5,4.5)(3,5.5)(0.5,4)};
			\node[above] at (5.5,4.6) {$B$};
			
			% U neighborhood
			\draw[-, thin] plot [smooth cycle] coordinates {(2.2,2.8)(3,2.2)(4.2,2.8)(4,4)(3,4.2)(2.3,3.8)};
			\node[above right] at (3.5,4.2) {$U$};
			
			% V neighborhood
			\draw[-, dashed] plot [smooth cycle] coordinates {(2.7,3.0)(3.2,2.7)(3.8,3.0)(3.7,3.6)(3.2,3.8)(2.8,3.5)};
			\node[left] at (2.8,3.4) {$V$};
			
			% Point a
			\coordinate (e) at (3.2,3.3);
			\filldraw [black] (e) circle (1pt) node[above] {$\ens$};
			
			% Points b_i in A (spread out)
			\coordinate (a1) at (1.2,1.5);
			\coordinate (a2) at (5.2,1.2);
			\coordinate (a3) at (4.8,4.2);
			\filldraw [black] (a1) circle (1pt) node[left] {$\ens[a]_1$};
			\filldraw [black] (a2) circle (1pt) node[left] {$\ens[a]_2$};
			\filldraw [black] (a3) circle (1pt) node[right] {$\ens[a]_3$};
			
			% c_i = pa + (1-p)b_i in V
			\coordinate (b1) at ($(e)!0.25!(a1)$);
			\coordinate (b2) at ($(e)!0.25!(a2)$);
			\coordinate (b3) at ($(e)!0.25!(a3)$);
			\filldraw [black] (b1) circle (1pt) node[below] {$\ens[b]_1$};
			\filldraw [black] (b2) circle (1pt) node[below] {$\ens[b]_2$};
			\filldraw [black] (b3) circle (1pt) node[above right] {$\ens[b]_3$};
			
			% p_i a + (1-p_i) b_i between a and c_i
			\coordinate (r1) at ($(e)!0.4!(b1)$);
			\coordinate (r2) at ($(e)!0.3!(b2)$);
			\coordinate (r3) at ($(e)!0.5!(b3)$);
			\filldraw [gray] (r1) circle (1pt);
			\filldraw [gray] (r2) circle (1pt);
			\filldraw [gray] (r3) circle (1pt);
			
			% Lines from a to b_i (dashed, showing the mixing direction)
			\draw[dotted, gray] (e) -- (a1);
			\draw[dotted, gray] (e) -- (a2);
			\draw[dotted, gray] (e) -- (a3);
			
			% Annotations
			\node[right] at (6.5,4.0) {$\ens[b]_i = p\ens+\bar{p}\ens[a]_i \in V$};
			\node[right] at (6.5,3.2) {\textcolor{gray}{$p_i\ens+\bar{p}_i\ens[a]_i = \lambda_i\ens+\bar{\lambda}_i\ens[b]_i \in U$}};
			\node[right] at (6.5,2.4) {$\lambda_i\ens+\bar{\lambda}_i\ens[b] \in U$ for all $\ens[b]\in V$, $\lambda_i \in [0,1]$};
		\end{tikzpicture}
		\caption{Boundedness shrinks all of $A$ into the neighborhood $V$ (giving the $\ens[b]_i$); the tube lemma guarantees that any further mixture of $\ens$ with a point of $V$ stays in $U$ (giving the gray points).}
	\end{figure}
	
	\begin{proof}
		Suppose $A$ is bounded around $\ens$. Let $U$ be an open neighborhood of $\ens$. Let $+_{\ens} : [0,1] \times \Ens \to \Ens$ be the map defined by $+_{\ens}(p, \ens[b]) \mapsto p \ens + \bar{\lambda} \ens[b]$. This map is continuous, and therefore $+_{\ens}^{-1}(U)$ is open with $[0,1]\times\{\ens[e]\} \subseteq +_{\ens}^{-1}(U)$. Since $[0,1]\times\{\ens[e]\}$ is compact, by the \href{https://en.wikipedia.org/wiki/Tube_lemma}{tube lemma}, there exists an open neighborhood $V$ of $\ens$ such that $p \ens + \bar{p} \ens[b] \in U$ for all $\ens[b] \in V$ and all $p \in [0,1]$.
