...
One of the issues that still needs to be closed is how to characterize classical spaces and classical contexts within a non-classical theory. Part of the problem is isolating the different parts that may lead to different features that one may consider classical.
...
We now want to find a characterization for a classical probability context: a set of ensembles that can be characterized by a family of classical probability measures. Since this set can be a subset of, for example, a quantum ensemble space and this property must work recursively, the first step is to restring the notion of decomposability to sets.
...
The next step is to show that each ensemble in a classical probability context can be understood as a probability measure and vice-versa. The issue is how to recover the set of points, the spectrum $\sigma(C)$ of the context, and its topology and $\sigma$-algebra $\Sigma_{\sigma(C)}$. This is still an open problem.
