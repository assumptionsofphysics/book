...
\begin{conj}[Convergence of submixtures (finite)]
	If $\sum_{i=1}^{\infty} p_i \ens_i$ converges, then $\sum_{i=2}^{\infty} \frac{p_i}{\bar{p_1}} \ens_i$ converges.
\end{conj}

\begin{conj}[Convergence of submixtures (infinite)]
...
\begin{conj}
	Let $\Ens$ be an ensemble space with a $T_1$ topology. Then the set of all internal points $I_{\Ens}$ is a convex set. The set of boundary points $B_{\Ens}$ is a closed set and therefore $I_{\Ens}$ is an open set.
\end{conj}
\begin{proof}
	Try to show that the limit of a sequence of boundary points cannot be an interior point.
\end{proof}
...
\begin{proof}
	...
	Since the arbitrary intersection of convex sets is a convex set, $\bigcap_i \hull(A_i)$ is a convex set. Therefore $\bigcap_i \hull(A_i) = \hull(\bigcap_i \hull(A_i))$. Note that a mixture of mixture of elements can always be re-expressed as a mixture of elements. TODO: this needs to be clarified. Moreover, an element that is the mixture of $A_i$ for all $i$ can be re-expressed as a mixture of the common elements, since $A_{i+1} \subset A_i$. Therefore $\hull(\bigcap_i \hull(A_i)) = \hull(\bigcap_i A_i)$. This means $\hull(\bigcap_i A_i) = \bigcap_i \hull(A_i)$, which proves the proposition.
\end{proof}
...
\begin{conj}
	Let $A \subset \Ens$ be a set of ensembles and let $\ens \in \hull(A)$ be an internal point with respect to $\hull(A)$. Then we can find  $\{\ens[a]_i\} \subseteq A$ and $p_i \in [0,1]$ such that $\ens = \sum_i p_i \ens[a]_i$.
\end{conj}

\begin{remark}
	Note that, given a set $A$, not all elements of its hull $\hull(A)$ can be understood as infinite mixtures. Let $\Ens$ bet the line segment $[0,1]$ and consider the set $A=\left\{\frac{1}{2^i}\right\}_{i=0}^{\infty}$. Every point in $(0,1]$ can be expressed as a finite mixture of two elements of $A$. For example, $1$ and any number smaller than the target number. However, zero cannot be expressed as a convex combination of positive numbers, and therefore it is not an infinite mixture of $A$. However, zero is the limit of the sequence, and therefore it will be in the topological closure of $A$.
\end{remark}

\begin{prop}
	The topological closure of the finite convex closure is closed under finite convex combinations.
\end{prop}

\begin{proof}
	Let $A \subseteq \Ens$ be a set closed under finite convex combinations. Let $\bar{A}$ be its topological closure. Let $\ens[a]_i \in A$ and $\ens[b]_i \in A$ be two sequences that converge to $\ens[a],\ens[b] \in \Ens$ respectively. Since $\bar{A}$ is the topological closure of $A$, then $\ens[a],\ens[b] \in \bar{A}$ given that they are limit points of elements of $A$.
	
	Consider a mixture $\ens = p \ens[a] + \bar{p} \ens[b]$ of the two limits and the sequence $\ens_i = p \ens[a]_i + \bar{p} \ens[b]_i$. Since $A$ is closed under convex combinations, $\ens_i \in A$. Since mixing is continuous, we have
	\begin{equation}
		\begin{aligned}
		...
		\end{aligned}
	\end{equation}
	Therefore $\ens \in \bar{A}$ since it is the limit point of a sequence of elements of $A$. This means that the topological closure of the finite convex closure is itself closed under finite convex combinations.
\end{proof}
...
It is still an open question whether the ensemble space embeds continuously in a topological vector space, and whether the topological vector space would have to be a locally convex.
