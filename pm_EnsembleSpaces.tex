...
\begin{mathSection}
\begin{prop}
	Let $\Ens$ be a discrete classical ensemble space. Then $\Ens$ is a probability context, the spectrum is the sample space and its topology is the discrete topology.
\end{prop}
...
\begin{proof}
	...
	Since the singletons are elements of the lattice, all ultrafilters are principal, meaning that they are all of the form $X(x) = \{A \in \mathfrak{L}^\separate(C) \, | \, \{x\} \in A\}$ where $x \in \Omega$ is an element of the sample space. Therefore the sample space and the spectrum coincide. Each singleton is an open set, since it is an element of the lattice. The topology is therefore the discrete topology.
\end{proof}

\begin{conj}
	Let $\Ens$ be a continuous classical ensemble space. Then $\Ens$ is a probability context, the spectrum is the symplectic manifold $X$ of all particle states and its topology is locally the standard topology of $\mathbb{R}^n$.
\end{conj}
\end{mathSection}
