...
\begin{proof}
	Since the upper entropy bound has to be the same for all spaces, let us assume that $\Ens$ is such that it contains countably many ensembles $\ens_i$ whose mixtures always saturate the upper entropy bound. That is, $S(p \ens[a] + \bar{p} \ens[b]) = I(p, \bar{p}) + p S(\ens[a]) + \bar{p} S(\ens[b])$ for all $\ens[a], \ens[b] \in \hull(e_i)$. Given associativity, for each $\ens[a] \in \hull(e_i)$ we will have $S(\sum p_i e_i ) = I_n(p_i) + \sum p_i S(\ens_i)$ where $I_n : \mathbb{R}^n \to \mathbb{R}$ is a function of the coefficient only. Not that, given commutativity, the order of the $p_i$ do not matter and, since the coefficients can be zero, we must have $I_n(p_i) = I_{n+1}(p_i, 0)$.
	
	We now show that $I_n\left({\frac{1}{N}}\right) = k \log N$ with $k > 0$. Pick two positive integers $n, m \in \mathbb{Z}^+$. Pick $nm$ elements of $a_{jk} \in \{e_i\}_{i=1}^{\infty}$ where $1\leq j \leq n$ and $1 \leq k \leq m$. We have:
	\begin{equation}
		\begin{aligned}
			S\left(\sum_{j=1}^{n}\sum_{k=1}^{m} \frac{1}{n}\frac{1}{m} \ens[a]_{jk}\right) &= I_{nm}\left(\left\{ \frac{1}{n}\frac{1}{m}\right\}_{i=1}^{nm}\right) + \sum_{j=1}^{n}\sum_{k=1}^{m} \frac{1}{n}\frac{1}{m} S\left(\ens[a]_{jk}\right) \\
			= S\left(\sum_{j=1}^{n} \frac{1}{n} \sum_{k=1}^{m} \frac{1}{m} \ens[a]_{jk}\right)  &=I_{n}\left(\left\{ \frac{1}{n}\right\}_{i=1}^{n}\right) + \sum_{j=1}^{n} \frac{1}{n} S\left(\sum_{k=1}^{m}\frac{1}{m}\ens[a]_{jk}\right) \\
			&=I_{n}\left(\left\{ \frac{1}{n}\right\}_{i=1}^{n}\right) + \sum_{j=1}^{n} \frac{1}{n}\left(I_{m}\left(\left\{ \frac{1}{m}\right\}_{i=1}^{m}\right) +  \sum_{k=1}^{m}\frac{1}{m}S\left(\ens[a]_{jk}\right)\right) \\
			&=I_{n}\left(\left\{ \frac{1}{n}\right\}_{i=1}^{n}\right) + I_{m}\left(\left\{ \frac{1}{m}\right\}_{i=1}^{m}\right) + \sum_{j=1}^{n} \sum_{k=1}^{m}\frac{1}{n}\frac{1}{m}S\left(\ens[a]_{jk}\right)
		\end{aligned}
	\end{equation}
	\begin{equation}
		\begin{aligned}
			I_{nm}\left(\left\{ \frac{1}{nm}\right\}_{i=1}^{nm}\right) 		&=I_n\left(\left\{ \frac{1}{n}\right\}_{i=1}^{n}\right) + I_m\left(\left\{ \frac{1}{m}\right\}_{i=1}^{m}\right)
		\end{aligned}
	\end{equation}
	Note that $f(n) = I_n\left(\left\{ \frac{1}{n}\right\}_{i=1}^{n}\right)$ is a function of $n$ only such that $f(nm) = f(n) + f(m)$. Since $f$ is continuous, we have $f(n) = k \log n$. Since the entropy is strictly concave, $k$ must be positive. Therefore
	\begin{equation}
		I_n\left(\left\{ \frac{1}{n}\right\}_{i=1}^{n}\right) = k \log n
	\end{equation}
	for some $k>0$.
	
	We now show that if the coefficients $p_i$ are rationals, $I_n\left(\left\{ p_i\right\}_{i=1}^{n}\right) = - k \sum_{i=1}^{n} p_i \log p_i$. Let $\{p_i\}_{i=1}^{n}$ be rational coefficients for a convex combination. We can write them as $p_i = \frac{m_i}{m}$ where $m$ is the minimum common denominator. Since $p_i$ form a convex combination, we must have $\sum_{i=1}^{n} m_i = m$. Since $m_i$ is a positive integer, we can write $m_i = \sum_{j=1}^{m_i} 1$. We now take $m$ orthogonal ensembles $\ens[a]_{ij}$ where $1 \leq i \leq n$ and $1 \leq j \leq m_i$. We have
	\begin{equation}
	\begin{aligned}
		S\left(\sum_{i=1}^{n}\sum_{j=1}^{m_i} \frac{1}{m} \ens[a]_{jk}\right) &= I_{m}\left(\left\{ \frac{1}{m}\right\}_{i=1}^{m}\right) + \sum_{i=1}^{n}\sum_{j=1}^{m_i} \frac{1}{m} S\left(\ens[a]_{ij}\right) \\
		&= k \log m + \sum_{i=1}^{n}\sum_{j=1}^{m_i} \frac{1}{m} S\left(\ens[a]_{ij}\right) \\
	\end{aligned}
	\end{equation}
	\begin{equation}
	\begin{aligned}
	= S\left(\sum_{i=1}^{n} \frac{m_i}{m} \sum_{j=1}^{m_i} \frac{1}{m_i} \ens[a]_{jk}\right)  &=I_{n}\left(\left\{ \frac{m_i}{m}\right\}_{i=1}^{n}\right) + \sum_{i=1}^{n} \frac{m_i}{m}  S\left(\sum_{j=1}^{m_i} \frac{1}{m_i} \ens[a]_{jk}\right) \\
		&=I_{n}\left(\left\{ p_i\right\}_{i=1}^{n}\right) + \sum_{i=1}^{n} \frac{m_i}{m}\left(I_{m_i}\left(\left\{ \frac{1}{m_i}\right\}_{i=1}^{m_i}\right) + \sum_{j=1}^{m_i} \frac{1}{m_i} S\left(\ens[a]_{ij}\right)\right) \\
		&=I_{n}\left(\left\{ p_i\right\}_{i=1}^{n}\right) + \sum_{i=1}^{n} \frac{m_i}{m} I_{m_i}\left(\left\{ \frac{1}{m_i}\right\}_{i=1}^{m_i}\right) + \sum_{i=1}^{n} \frac{m_i}{m} \sum_{j=1}^{m_i} \frac{1}{m_i}S\left(\ens[a]_{jk}\right) \\
		&=I_{n}\left(\left\{ p_i\right\}_{i=1}^{n}\right) + \sum_{i=1}^{n} p_i k \log m_i + \sum_{i=1}^{n}  \sum_{j=1}^{m_i} \frac{1}{m} S\left(\ens[a]_{jk}\right)
	\end{aligned}
	\end{equation}
	\begin{equation}
	\begin{aligned}
		k \log m &= I_{n}\left(\left\{ p_i\right\}_{i=1}^{n}\right) + \sum_{i=1}^{n} p_i k \log m_i \\
		I_{n}\left(\left\{ p_i\right\}_{i=1}^{n}\right) &= k \log m - \sum_{i=1}^{n} p_i k \log m_i = \sum_{i=1}^{n} p_i k \log m - \sum_{i=1}^{n} p_i k \log m_i \\
		&= - \sum_{i=1}^{n} p_i k \log \frac{m_i}{m} = - k \sum_{i=1}^{n} p_i \log p_i
	\end{aligned}
	\end{equation}
	
	Lastly, let $\{p_i\}_{i=1}^{n}$ be coefficients for a convex combination, not necessarily rational. Since $I_n$ is continuous and $p_i$ can be approximated with rational values at an arbitrary level of precision, we will have $I_n\left(\left\{ p_i\right\}_{i=1}^{n}\right) = - k \sum_{i=1}^{n} p_i \log p_i$. In the case of $n=2$, we have $I_2(p_1, p_2) = I(p_1, p_2) = - k p_1 \log p_i - k p_2 \log p_i$.
