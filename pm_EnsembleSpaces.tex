...
\begin{prop}
	...
	\begin{enumerate}
		...
		\item the state capacity is continuous from above and below.
	\end{enumerate}
\end{prop}
...
\begin{proof}
	...
	5. Let $\{A_i\} \subseteq \Sigma_{\Ens}$ be an increasing sequence. Since $\scap$ is monotone, $\scap(A_i)$ is also increasing. Since $\hull$ is continuous from below, we have:
	\begin{equation}
		\begin{aligned}
			\scap(A) &= \sup(2^{S(\hull(A))}\cup\{0\}) = \sup(2^{S(\bigcup\hull(A_i))}\cup\{0\})\\
			&= \sup(\bigcup 2^{S(\hull(A_i))}\cup\{0\}) = \sup(\bigcup \scap(A_i)).
		\end{aligned}
	\end{equation}
	But since $\scap(A_i)$ is increasing, the supremum is exactly the limit. Therefore the state capacity is continuous from below.
	
	Since $\hull$ is also continuous from above, the same argument works for decreasing sequences by inverting the order.
\end{proof}
