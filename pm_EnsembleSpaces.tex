...
\begin{defn}
	Let $\Ens$ be an ensemble space and $X \subseteq \Ens$ be a subset. Then $\dist(X) \subseteq \Ens$ is the the set of all ensembles that are distinct from all elements of $X$. An \textbf{ensemble subspace} is a subset $X \in \Ens$ such that $X = \dist(\dist(X))$.
\end{defn}
