...
\begin{prop}
	Let $\Ens$ be a discrete or classical ensemble space over a sample space $X$ and $\mu$ the corresponding measure. Then $A \subseteq \Ens$ is an $\ortho$-subspace if and only if there exists $U \subseteq X$ such that $A = \{ p \in \Ens \, | \, p(U) = 1\}$.
\end{prop}

\begin{proof}
	Recall that in a classical ensemble space, $p \ortho \lambda$ if and only if $\lambda(\support(p)^{\complement}) = 1$. Let $A \subseteq \Ens$ be an $\ortho$-subspace. Let $U = \bigcup_{p \in A} \support(p)$ be the union of all supports. Then $\lambda \in A^{\ortho}$ is orthogonal to all elements of $A$ if and only if $\lambda(U^{\complement}) = 1$. Conversely, $p \in A$ only if it is orthogonal to all elements, which means if and only if $p(U) = 1$. Therefore if $A$ is a subspace, there is a $U \subseteq X$ such that $p(U)=1$ for all $p \in A$.
	
	Conversely, let $A = \{ p \in \Ens \, | \, p(U) = 1\}$ for some $U \subseteq X$. The orthogonal complement will be the set of all probability measure $\lambda \in \Ens$ such that $\lambda(U^{\complement})=1$. The complement of the complement will return $A$, which means $A = (A^{\ortho})^{\ortho}$ is a subspace.
\end{proof}
...
\begin{prop}
	Let $\Ens$ be a quantum ensemble space over a Hilbert space $\mathcal{H}$. Then $A \subseteq \Ens$ is an $\ortho$-subspace if and only if there exists a subspace $U \subseteq \mathcal{H}$ with a corresponding projector $P$ such that $A = \{ \rho \in \Ens \, | \, \tr[P\rho] = 1\}$.
\end{prop}

\begin{proof}
	Given a density operator $\rho \in \Ens$, recall that its support is the subspace spanned by the non-zero eigenvectors. With a slight abuse of notation, since each subspace corresponds to a projector, $\support(\rho)$ will also note the projector corresponding to the support and the join/meet/complement of projectors will correspond to the projector of the join/meet/complement of the subspaces.
	
	For a quantum ensemble space, $\rho \ortho \sigma$ if and only if $\tr[ \support(\rho)^{\ortho} \sigma] = 1$. Let $A \subseteq \Ens$ be an $\ortho$-subspace. Let $P = \bigvee_{\rho \in A} \support(p)$ be the projector corresponding to the subspaces $U$ spanned by all supports. Then $\sigma \in A^{\ortho}$ is orthogonal to all elements of $A$ if and only if $\tr[\sigma P^{\ortho}] = 1$. Conversely, $\rho \in A$ if it is orthogonal to all elements, which means if and only if $\tr[\rho P] = 1$. Therefore if $A$ is a subspace, there is a projector $P$ such that $\tr[\rho P] = 1$ for all $\rho \in A$.
	
	Conversely, let $A = \{ \rho \in \Ens \, | \, \tr[\rho P] = 1\}$ for some projector $P$. The orthogonal complement $A^{\ortho}$ will be the set of all density operators $\sigma \in \Ens$ such that $\tr[\sigma P^{\ortho}]=1$. The complement of the complement will return $A$, which means $A = (A^{\ortho})^{\ortho}$ is a subspace.
\end{proof}


Note that, in general, the two notion of subspaces, and therefore their lattice, will be different.  If a classical ensemble space is exactly an ensemble space for which orthogonality and separateness coincide, the two lattices will coincide if and only if we have a classical space.
