...
\begin{defn}
	...
	\begin{enumerate}
		\item non-negative: $\mu(U) \in [0 , +\infty]$
		...
		\item locally finite: for any $U$, we can find $V \in \textsf{T}_X$ such that $V \subseteq U$ and $\mu(V) \in (0, + \infty)$.
	\end{enumerate}
...
\begin{coro}
	...
	\begin{enumerate}
		...
		\item given an infinite open set $U$, there exists an increasing sequence of finite open sets $U_i$ such that $U_i \to U$ and $\mu(U_i) \to +\infty$.
	\end{enumerate}
\end{coro}
...
TODO: make sure that nicely boundaried sets are A and B
...
\begin{mathSection}
	...
	\begin{conj}
		The lattice is a complemented bounded lattice, not necessarily a Boolean algebra.
	\end{conj}
	\begin{proof}
		Distributivity fails as complement may not be unique (e.g. lattice of orthogonal subspaces of vector space).
	\end{proof}
...
\subsection{Space of differences}

The idea is to actually construct the affine space through the notion of differences.
...
\begin{remark}
	Note that any equation $\sum_{i=1}^n r_i \ens[a]_i = \sum_{i=1}^n s_i \ens[b]_i$ such that $\sum_{i=1}^n r_i = \sum_{i=1}^n s_i = t$ can be understood as an equation between affine combinations by simply dividing both sides by $t$. From now on, all equations of such kind should be understood that way.
\end{remark}

\begin{defn}
	An \textbf{interior point} of an ensemble space is an ensemble $\ens \in \Ens$ such that for every $\ens[a] \in \Ens$ there exists a $\ens[b] \in \Ens$ and a $p \in (0,1)$ such that $\ens = p \ens[a] + \bar{p} \ens[b]$. 
\end{defn}


\begin{defn}[Ensemble differences]
	Given an ensemble space, a difference between two ensemble represents the change required to transform one ensemble into another. Formally, an \textbf{ensemble difference} is an ordered pair of ensembles $\ens[a],\ens[b] \in \Ens$, noted $\ens[b] - \ens[a]$. A \textbf{scaled difference}, noted $r(\ens[b] - \ens[a])$, is a pair formed by a real number $r \in \mathbb{R}$ and a difference.
\end{defn}

\begin{defn}
	Two scaled differences are \textbf{equivalent}, noted $r(\ens[b] - \ens[a]) \sim s(\ens[c] - \ens[a])$, if $r\ens[b] - r \ens[a] = s\ens[c] - s \ens[a]$.
\end{defn}

\begin{prop}
	The above relation is reflexive, symmetric and transitive and therefore is an equivalence relation.
\end{prop}

\begin{proof}
	For reflexivity, we have $r \ens[b] - r \ens[a] = r \ens[b] + r \ens[a]$, which is satisfied for any $\ens[a], \ens[b] \in \Ens$ and $r \in \mathbb{R}$.
	
	For symmetry, note that right hand side of the equality can be obtained by switching $r$ with $s$ and $\ens[b]$ with $\ens[c]$ in left right hand side.
	
	For transitivity, suppose $r(\ens[b] - \ens[a]) \sim s(\ens[c] - \ens[a])$ and $s(\ens[c] - \ens[a]) \sim t(\ens[d] - \ens[a])$. We have $r\ens[b] - r \ens[a] = s \ens[c] - s \ens[a]$ and $s\ens[c] - s \ens[a] = t\ens[d] - t \ens[a]$, or, equivalently, $\ens[b] = \frac{r-s}{r} \ens[a] +\frac{s}{r}\ens[c]$ and $\ens[c] = \frac{s-t}{s} \ens[a] +\frac{t}{s}\ens[d]$. Putting them together, $\ens[b] = \frac{r-s}{r} \ens[a] +\frac{s}{r}\left(\frac{s-t}{s} \ens[a] +\frac{t}{s}\ens[d]\right) = \frac{r-s + s -t}{r} \ens[a] +\frac{t}{r}\ens[d] = \frac{r-t}{r} \ens[a] +\frac{t}{r}\ens[d]$. Which means $r\ens[b] - r \ens[a] = t \ens[d] - t \ens[a]$. Therefore $r(\ens[b] - \ens[a]) \sim t(\ens[d] - \ens[a])$.
\end{proof}

\begin{coro}
	The equivalence relationship satisfies the following:
	\begin{enumerate}
		\item the equivalence class $\left[r(\ens[b] - \ens[a])\right]$ is given by the set $\left\{s\left(\frac{r}{s}\ens[b] + \frac{s-r}{s}\ens[a] - \ens[a]\right)\right\}$
		\item $r(\ens[b] - \ens[a]) \sim s(\ens[c] - \ens[a])$, then $\ens[a]$, $\ens[b]$ and $\ens[c]$ are on the same line
		\item given $p \in [0,1]$, $r(\ens[b] - \ens[a]) \sim \frac{r}{p}(p\ens[b] + \bar{p}\ens[a] - \ens[a])$
		\item $r(\ens[b] - \ens[a]) \sim -r(\ens[c] - \ens[a])$ if $\frac{1}{2}\ens[b] + \frac{1}{2} \ens[c] = \ens[a]$
		\item $r(\ens[b] - \ens[a]) \sim s(\ens[b] - \ens[a])$ with $\ens[b] \neq \ens[a]$ if and only if $r=s$
		\item $r(\ens[b] - \ens[a]) \sim s(\ens[b] - \ens[a])$ with $\ens[b] = \ens[a]$ for any $r,s \in \mathbb{R}$
	\end{enumerate}
\end{coro}

\begin{proof}
	For 1, note that the condition for equivalence is $r \ens[b] - r \ens[a] = s\ens[c] - s \ens[a]$. This can be re-expressed as $\ens[c] = \frac{r}{s}\ens[b] + \frac{s-r}{s} \ens[a]$.
	
	For 2, since 1 tells us that $\ens[c]$ is an affine combination of $\ens[a]$ and $\ens[b]$, it must be on the same line.
	
	For 3, just substitute $s= \frac{r}{p}$ in 1.
	
	For 4, if we substitute $s = -r$ in the definition, we have $r \ens[b] - r \ens[a] = -r \ens[c] + r \ens[a]$, which means $\frac{1}{2}\ens[b] + \frac{1}{2} \ens[c] = \ens[a]$.
	
	For 5 and 6, suppose $r(\ens[b] - \ens[a]) \sim s(\ens[b] - \ens[a])$. Then $r\ens[b] + s\ens[a] = s \ens[b] + r \ens[a]$, which is an equality between affine combinations. If $\ens[b] \neq \ens[a]$, the two affine combinations correspond to the same ensemble if and only if $r=s$. If $\ens[b] = \ens[a]$, then the equality is always satisfied.
\end{proof}


\begin{defn}
	The \textbf{addition} between two equivalence classes of differences is defined by 
	$$\left[r(\ens[b] - \ens[a])\right] + \left[s(\ens[c] - \ens[a])\right] = \left[(r+s) (\frac{r}{r+s}\ens[b] + \frac{s}{r+s} \ens[c] - \ens[a])\right] = \{ \}$$
	where $\frac{r}{r+s}\ens[b] + \frac{s}{r+s} \ens[c]$ is an affine combination. Two scaled differences are \textbf{equivalent} if their difference is the reference ensemble. That is
	$$r(\ens[b] - \ens[a]) \sim s(\ens[c] - \ens[a]) \textrm{ if } r(\ens[b] - \ens[a]) - s(\ens[c] - \ens[a]) = (r+s) (\ens[a] - \ens[a])$$
	
	
	On the scaled differences is defined an equivalence relationship $\sim$ that is the closure of following relationships
	\begin{enumerate}
		\item $r(\ens[b] - \ens[a]) \sim_1 s(\ens[c] - \ens[a])$ if $r,s \in [0, +\infty)$ and $\frac{r}{r+s}\ens[b] + \frac{s}{r+s} \ens[a] = \frac{s}{r+s}\ens[c] + \frac{r}{r+s} \ens[a]$ 
		\item $r(\ens[b] - \ens[a]) \sim_2 -r(\ens[c] - \ens[a])$ if $\frac{1}{2}\ens[b] + \frac{1}{2} \ens[c] = \ens[a]$
	\end{enumerate}
\end{defn}
\begin{proof}
	For the addition to be well-defined, we need to make sure that the affine combination is always well-defined. If $r$ and $s$ have the same sign, 
\end{proof}
...
\begin{coro}
	...
	\begin{enumerate}
		\item 
		\item $r(\ens[b] - \ens[a]) \sim \frac{r}{p}(p\ens[b] + \bar{p}\ens[a] - \ens[a])$ for all $p \in [0,1]$
		\item $r(\ens[b] - \ens[a]) \sim s(\ens[b] - \ens[a])$ with $\ens[b] \neq \ens[a]$ if and only if $r=s$
		\item $r(\ens[b] - \ens[a]) \sim s(\ens[b] - \ens[a])$ with $\ens[a] \neq \ens[a]$ for any $r,s \in \mathbb{R}$
		\item the addition is consistent with $\sim_1$
	\end{enumerate}
\end{coro}

\begin{proof}
	For 1, first we verify $\sim_1$ holds. Let $r \geq 0$ We have $r,\frac{r}{p} \in [0, + \infty)$ and $\frac{r}{p} \frac{1}{r + \frac{r}{p}} (p\ens[b] + \bar{p}\ens[a]) + \frac{r}{r+\frac{r}{p}}  \ens[a] = \frac{r}{r+\frac{r}{p}} \ens[b] + \left(\frac{r- rp}{p} +r\right) \frac{1}{r + \frac{r}{p}} \ens[a] = \frac{r}{r+\frac{r}{p}} \ens[b] + \frac{r}{p} \frac{1}{r + \frac{r}{p}} \ens[a]$. Now, suppose $r<0$
...
\begin{remark}
	Note that the notation is chosen so that, effectively, it is like we had operations on the actual ensembles. For example $k(\ens[b] - \ens[a]) = k\ens[b] - k \ens[a] = k\ens[b] - k \ens[a] + \ens[a] - \ens[a] = k\ens[b] + (1 - k) \ens[a] - \ens[a]= (k\ens[b] - \bar{k}\ens[a]) - \ens[a]$. This can be done because the ensemble space does embed into a vector space. Note that the second term in the difference plays the role of the origin, so all operations are relative to that. For example $\frac{1}{2}(\ens[b] - \ens[a]) + \frac{1}{2}(-(\ens[b] - \ens[a])) = \frac{1}{2}(\ens[b] - \ens[a]) + \frac{1}{2}(\ens[c] - \ens[a])) = \frac{1}{2}(\ens[b] + \ens[c]) - \ens[a] = \ens[a] - \ens[a]$, which shows that the sum of opposite differences is ``zero''. The reason why this is an affine structure and not a vector space is precisely because the vectors are differences: there is no physically operation that corresponds to multiplying an ensemble by a constant.
\end{remark}

\begin{defn}
	$r(\ens[b] - \ens[a]) \sim -r(\ens[c] - \ens[a])$ if $\frac{1}{2}\ens[b] + \frac{1}{2} \ens[c] = \ens[a]$
	
	$r(\ens[b] - \ens[a]) \sim s(\ens[c] - \ens[a])$ if $\frac{r}{r+s}\ens[b] + \frac{s}{r+s} \ens[a] = \frac{s}{r+s}\ens[c] + \frac{r}{r+s} \ens[a]$
	
	$r(\ens[b] - \ens[a]) + s(\ens[c] - \ens[a]) = (r+s) (\frac{r}{r+s}\ens[b] + \frac{s}{r+s} \ens[c] - \ens[a])$
\end{defn}

\begin{proof}
	The zero element is $0 = (\ens[a] - \ens[a])$. $(\ens[b] - \ens[a]) + (\ens[a] - \ens[a]) = 2 (\frac{1}{2}\ens[b] + \frac{1}{2} \ens[a] - \ens[a])$.
\end{proof}
