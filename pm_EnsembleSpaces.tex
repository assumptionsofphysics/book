...
Intuitively, we would expect the fraction $\fraction_{\ens}(\ens[a])$ to be continuous both in $\ens[a]$ and $\ens$, but this is not the case. For example, as shown in Fig. \ref{pm_es_figFractionDiscontinuous}, let $\Ens$ be the Bloch ball and let $\ens[a]_i$ be a sequence of pure states (i.e. points on the surface) that converge to another pure state $\ens$. Since they are all extreme points, we have $\fraction_{\ens}(\ens[a]_i) = 0$ and $\fraction_{\ens[a]_i}(\ens) = 0$. However, $\fraction_{\ens}(\ens) = 1$. This is the recurring problem of the non-continuity at the edges. The following conjecture, however, may still be true:
\begin{conj}
