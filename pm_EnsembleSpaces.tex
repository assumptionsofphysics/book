	...
	\begin{remark}
		...
		\begin{figure}[H]
			...
			\begin{tikzpicture}
				...
				\draw[dashed] (d) -- (e);
			\end{tikzpicture}
			\caption{The set $\{\ens[a], \ens[b]\}$ is componential, but its convex hull (the segment) is not: $\ens[c]$ is on the segment but can be decomposed into $\ens[d]$ and $\ens[e]$, which lie outside.}
		\end{figure}
	\end{remark}
\end{mathSection}
