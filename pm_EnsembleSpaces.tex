...
Separateness and orthogonality are tightly related to each other. First of all, orthogonality implies separateness and is therefore a stronger properties. Moreover, orthogonality is an irreflexive symmetric relation, like separateness.
...
\begin{prop}
	...
	\begin{enumerate}
		...
		\item imply separateness: if $\ens[a] \ortho \ens[b]$ then $\ens[a] \separate \ens[b]$.
	\end{enumerate}
\end{prop}
...
\begin{proof}
	...
	For 3, let $\ens[a] = p \ens[b] + \bar{p} \ens[c]$. By mixtures preserve orthogonality, $\ens[b] \ortho \ens[a]$ if and only if $\ens[b] \ortho \ens[b]$ and $\ens[b] \ortho \ens[c]$. But $\ens[b]$ is not orthogonal to itself, therefore $\ens[a]$ and $\ens[b]$ are not orthogonal.
	
	For 4, we demonstrate the contrapositive: that ensembles that are not separate are not orthogonal. Let $\ens[a], \ens[b] \in \Ens$ have a common component. That is, $\ens[a] = p \ens[c] + \bar{p} \ens[d]$ and $\ens[b] = \lambda \ens[c] + \bar{\lambda} \ens[e]$. By mixtures preserve orthogonality, $\ens[a] \ortho \ens[b]$ if and only if $\ens[a] \ortho \ens[c]$ and $\ens[a] \ortho \ens[e]$. But $\ens[c]$ is a component of $\ens[a]$, therefore they are not orthogonal. Therefore two ensemble that have a common component are not orthogonal. This means that if two ensembles are orthogonal they cannot have a common component and are therefore separate.
\end{proof}
...
\begin{defn}
	A convex set $V \subseteq \Ens$ is finite dimensional if it embeds into a finite dimensional subspace of the embedding vector space. A finite dimensional convex set $V$ is a subset of $\mathbb{R}^n$ and can be charted by a set of coordinates, even though it is not a manifold (i.e. it may have a boundary, a boundary that is not a manifold, corners and so on). We can identify a set of coordinate that are smooth with respect to the mixing coefficients (i.e. to the affine structure). We call that that the differentiable structure of $V$. A \textbf{linear chart} is a chart that preserves the linearity of the affine structure. A \textbf{variation} $\delta \ens$ is assumed to be defined on the affine structure, though it can be re-expressed in any coordinate of the differentiable structure.
\end{defn}
...
\begin{thrm}
	Let $\Ens$ be a smooth ensemble space. Then, in a linear chart, we have
