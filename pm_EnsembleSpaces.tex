...
\begin{defn}
	Given a function $f : \Ens \to \mathbb{R}$, the its \textbf{convex supremum} is the set function to the extended real line defined as $cs_f(A) = \sup(f(\hull(A)) \cup \{\inf(f(\Ens))\})$ that returns the highest value of $f$ reachable by convex combinations of $A$.
\end{defn}

\begin{prop}\label{pm_es_convexSupremumProps}
	For any $f$, the convex supremum $cs_f$ has the following properties
	\begin{enumerate}
		\item range of $f$: $cs_f(A) \in [\inf(f(\Ens)), \sup(f(\Ens))]$
...
\begin{proof}
	1. If $A$ is non-empty, $cs_f(A)$ returns the supremum of the subset of value returned by $f(A)$, which means $\inf(f(\Ens)) \leq cs_f(A) \leq \sup(f(\Ens))$. If $A$ is empty, $\inf(f(\Ens)) = cs_f(A) \leq \sup(f(\Ens))$. Therefore $cs_f(A) \in [\inf(f(\Ens)), \sup(f(\Ens))]$ for all $A$.

	2. Since the $\hull$ is an increasing function, the image of a set through a map is an increasing function and the supremum is an increasing function, the convex supremum is an increasing function.
...
\begin{remark}
	Note that the state capacity coincides with the convex supremum of the exponential of the entropy, except for the empty set.
\end{remark}
...
\begin{proof}
	1. The exponential can only return non-negative values, therefore the state capacity of a set is non-negative. 
	
	2. Since all convex supremum maps are increasing, the state capacity is increasing for all sets that are not the empty set. Since $0 \leq \inf(2^{S(\Ens)})$, the function is increasing over the empty set.
	...
	5. Since convex supremum maps are continuous from below, the state capacity is continuous from below.
\end{proof}
...
\begin{mathSection}
	...
	\begin{remark}
		Note that the fraction capacity is the convex supremum of the capacity for any ensemble space with at least two ensembles. An open question is whether the type of hull matters. If the fraction is proven to be continuous, then it does not matter.
	\end{remark}
	...
	\begin{proof}
		...
		2. Since all convex supremum maps are increasing, the fraction capacity is increasing.
		...
		4. Since convex supremum maps are continuous from below, the fraction capacity is continuous from below.
	\end{proof}
