...
\begin{mathSection}
	...
	\begin{proof}
		...
		For 1, since the lattice is a $\bigcap$-structure, $A \wedge A^{R} = A \cap A^{R}$ and from \ref{pm_ensmblespaces_symirreflproperties} we have $A \cap A^{R} = \emptyset$. Since the lattice is a $\bigcap$-structure, $A \vee A^{R} \supseteq A \cup A^{R}$. Using \ref{pm_es_rComplProps} and \ref{pm_ensmblespaces_symirreflproperties} we have $((A \vee A^{R})^{R})^{R} \supseteq ((A \cup A^{R})^{R})^{R} = (\emptyset)^{R} = X$.
	\end{proof}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\draw[fill=ensembleFill, thick] (a) -- (e2) node[midway,left] {$A$} -- (e3) -- (b) node[midway,right] {$B$} -- cycle;
		\filldraw [black] (a) circle (1pt) node[above left] {$\ens[a]$};
		...
		\node (top) at (6,3) {$\top = \Ens$};
		\node (a) at (5,2) {$A$};
		\node (b) at (7,2) {$B$};
		\node (x) at (5,1) {$X=\{\ens[2]\}$};
		\node (y) at (7,1) {$Y=\{\ens[3]\}$};
		\node (bot) at (6,0) {$\bot = \emptyset$};
		\draw (bot) -- (x) -- (a) -- (top) -- (b) -- (y) -- (bot);
	\end{tikzpicture}
	%\includegraphics[width=0.7\textwidth]{tempimages/CutTriangleSubspaces.jpg}\label{pm_es_figCutTriangleOrthoSubspaces}
	\caption{On the left, the cut triangle. On the right, the lattice of the orthogonal subspaces.}
\end{figure}

The lack of orthomodularity is due to the multiple complements that where one is a subset of the other. This means that $X \vee (X^\perp \wedge A) = X \vee (B \wedge A) = X \vee \bot = X$ instead of $A$, if the lattice were orthomodular. This leads to the following:
\begin{conj}
...
To show that the mixing operation is continuous, it suffices to show that the inverse image of sets of the form $(r,1]$ is open. To do that, we show that each point in $+^{-1}((r,1]) = \{(p, a, b) \int [0,1]^3 \, | \, pa+\bar{p}b > r \}$ has an open neighborhood. First, note that sets of the form $(p,1]\times(a,1]\times(b,1]$ and $[0,p)\times(a,1]\times(b,1]$ are open because they are the product of open sets. Next note that if $pa+\bar{p}b>m$, then if $a>=b$, for any $\lambda>p$, $c>a$ and $d>b$, $\lambda c+\bar{\lambda}d>m$. This is because the convex combination can only increase if the elements increase and/or if the coefficient of the greater element increases. In terms of sets, if $pa+\bar{p}b>m$ and $a>=b$, $+((p,1],(a,1],(b,1]) \subseteq (m,1]$. Conversely, if $a<=b$, $+([0,p),(a,1],(b,1]) \subseteq (m,1]$. Now, let $pa+\bar{p}b=m>r$. Then $pa+\bar{p}b>\frac{m+r}{2}$. If $a>=b$, $+((p,1],(a,1],(b,1]) \subseteq \left(\frac{m+r}{2},1\right]\subseteq (m,1]$. If $a<b$,  $+([0,p),(a,1],(b,1]) \subseteq \left(\frac{m+r}{2},1\right]\subseteq (m,1]$. Therefore, every point in $+^{-1}((r,1])$ has an open neighborhood, and $+^{-1}((r,1])$ is an open set. This means that the mixing operation is continuous.
