...
\begin{defn}
	Let $\Ens$ be an ensemble space and $X \subseteq \Ens$ be a subset. The \textbf{distinct complement} $X^\perp \subseteq \Ens$ is the the set of all ensembles that are distinct from all elements of $X$. An \textbf{ensemble subspace} is a subset $X \in \Ens$ such that $X = (X^\perp)^\perp$.
\end{defn}
...
\begin{proof}
	...
	The set $\mathfrak{L}$ is a collections of subsets of $X$. Let $\{U_i\}_{i \in I} \subseteq \mathfrak{L}$ be a non-empty family. Then $\bigcap_{i \in I} U_i = \bigcap_{i \in I} (U_i^{RC})^{RC} = ((\bigcap_{i \in I} U_i)^{RC})^{RC}$. Therefore $\bigcap_{i \in I} U_i \in \mathfrak{L}$ by the third property of \ref{pm_ensmblespaces_symirreflproperties}. This means $\mathfrak{L}$ is an $\bigcap$-structure. Using the second property of \ref{pm_ensmblespaces_symirreflproperties}, $X = \emptyset^{RC} = (X^{RC})^{RC}$. Therefore $\mathfrak{L}$ is a topped $\bigcap$-structure. This also means that it is a complete lattice.
\end{proof}

\begin{defn}
	Let $\Ens$ be an ensemble space and $X \subseteq \Ens$ an ensemble subspace. The \textbf{dimension} of the subspaces is defined as $\dim(X) = \sup(\{S(\ens) \, | \, \ens \in X\})$.
\end{defn}
