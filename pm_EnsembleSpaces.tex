...
Summary: Assume WLOG $F(\ens_1) \geq F(\ens_2)$. For any $\ens' = \alpha\ens[a] + \beta\ens_1 + \gamma\ens_2 \in \hull({\ens[a], \ens_1, \ens_2})$, write $\ens' = (\alpha+\gamma)\ens[d] + \beta\ens_1$ where $\ens[d] = \frac{\alpha}{\alpha+\gamma}\ens[a] + \frac{\gamma}{\alpha+\gamma}\ens_2$. Since $F(\ens[a]) \leq F(\ens_1)$ and $F(\ens_2) \leq F(\ens_1)$, we have $F(\ens[d]) \leq F(\ens_1)$. By $\ens_1 \in S_f$: $F(\face({\ens[d], \ens_1})) \subseteq (-\infty, f]$. Since $\ens' \in \hull({\ens[d], \ens_1})$, every component of $\ens'$ is in $\face({\ens[d], \ens_1})$ and therefore has $F \leq f$.
Since this holds for all $\ens' \in \hull({\ens[a], \ens_1, \ens_2})$, we have $F(\face({\ens[a], \ens_1, \ens_2})) = F(\component(\hull({\ens[a], \ens_1, \ens_2}))) \subseteq (-\infty, f]$.
And since $\face({\ens[a], \ens}) = \face({\ens[a], \ens_1, \ens_2})$, we have $F(\face({\ens[a], \ens})) \subseteq (-\infty, f]$. Therefore $\ens \in S_f$.


\begin{mathSection}
	\begin{defn}
		Let $\Ens$ be an ensemble space and $F : \Ens \to \mathbb{R}$ a statistical quantity. An ensemble $\ens \in \Ens$ is \textbf{supported by $(-\infty, f]$ of $F$} if for all $\ens[a] \in \Ens$ with $F(\ens[a]) \leq F(\ens)$, we have $F(\face(\{\ens[a], \ens\})) \subseteq (-\infty, f]$. The set of all ensembles supported by $(-\infty, f]$ of $F$ is noted $S_f^F$, or simply $S_f$ when $F$ is clear from context.
	\end{defn}
	
	\begin{prop}
		The set $S_f$ is componential.
	\end{prop}
	
	\begin{proof}
		Let $\ens \in S_f$ and $\ens = p\ens[c] + \bar{p}\ens[b]$ for some $p \in (0,1)$ and $\ens[c], \ens[b] \in \Ens$. We show that $\ens[c] \in S_f$. Let $\ens[a] \in \Ens$ with $F(\ens[a]) \leq F(\ens[c])$. We need to show that $F(\face(\{\ens[a], \ens[c]\})) \subseteq (-\infty, f]$.
		
		Let $\ens[d] = p\ens[a] + \bar{p}\ens[b]$. By linearity of $F$ and $F(\ens[a]) \leq F(\ens[c])$:
		\begin{equation}
			F(\ens[d]) = pF(\ens[a]) + \bar{p}F(\ens[b]) \leq pF(\ens[c]) + \bar{p}F(\ens[b]) = F(\ens).
		\end{equation}
		Since $\ens[d] = p\ens[a] + \bar{p}\ens[b]$, $\ens[a]$ is a component of $\ens[d]$ and therefore $\ens[a] \in \face(\{\ens[d]\}) \subseteq \face(\{\ens[d], \ens\})$. Since $\ens = p\ens[c] + \bar{p}\ens[b]$, $\ens[c]$ is a component of $\ens$ and therefore $\ens[c] \in \face(\{\ens\}) \subseteq \face(\{\ens[d], \ens\})$. Since $\face(\{\ens[d], \ens\})$ is a face containing both $\ens[a]$ and $\ens[c]$, by minimality $\face(\{\ens[a], \ens[c]\}) \subseteq \face(\{\ens[d], \ens\})$.
		
		Since $F(\ens[d]) \leq F(\ens)$ and $\ens \in S_f$, we have $F(\face(\{\ens[d], \ens\})) \subseteq (-\infty, f]$. Therefore $F(\face(\{\ens[a], \ens[c]\})) \subseteq F(\face(\{\ens[d], \ens\})) \subseteq (-\infty, f]$.
	\end{proof}

	\begin{prop}
		The set $S_f$ is convex.
	\end{prop}
	
	\begin{proof}
		Let $\ens_1, \ens_2 \in S_f$ and $\ens = q\ens_1 + \bar{q}\ens_2$ for some $q \in [0,1]$. We show that $\ens \in S_f$. Let $\ens[a] \in \Ens$ with $F(\ens[a]) \leq F(\ens)$. We need to show that $F(\face(\{\ens[a], \ens\})) \subseteq (-\infty, f]$.
		
		Since $\ens_1$ and $\ens_2$ are components of $\ens$, we have $\ens_1, \ens_2 \in \face(\{\ens\}) \subseteq \face(\{\ens[a], \ens\})$. Therefore $\face(\{\ens[a], \ens_1, \ens_2\}) \subseteq \face(\{\ens[a], \ens\})$. Conversely, since $\ens \in \hull(\{\ens_1, \ens_2\}) \subseteq \face(\{\ens[a], \ens_1, \ens_2\})$, we have $\face(\{\ens[a], \ens\}) \subseteq \face(\{\ens[a], \ens_1, \ens_2\})$. Therefore $\face(\{\ens[a], \ens\}) = \face(\{\ens[a], \ens_1, \ens_2\}) = \component(\hull(\{\ens[a], \ens_1, \ens_2\}))$.
		
		Without loss of generality, assume $F(\ens_1) \geq F(\ens_2)$. Since $F(\ens[a]) \leq F(\ens) \leq F(\ens_1)$ and $F(\ens_2) \leq F(\ens_1)$, any convex combination of $\ens[a]$ and $\ens_2$ has $F$-value at most $F(\ens_1)$.
		
		Let $\ens' = \alpha\ens[a] + \beta\ens_1 + \gamma\ens_2 \in \hull(\{\ens[a], \ens_1, \ens_2\})$ with $\alpha + \beta + \gamma = 1$. We can rewrite $\ens' = \beta\ens_1 + \bar{\beta}\ens[d]$ where $\ens[d] = \frac{\alpha}{\bar{\beta}}\ens[a] + \frac{\gamma}{\bar{\beta}}\ens_2$ is a convex combination of $\ens[a]$ and $\ens_2$. Therefore $F(\ens[d]) \leq F(\ens_1)$. Since $\ens_1 \in S_f$, we have $F(\face(\{\ens[d], \ens_1\})) \subseteq (-\infty, f]$. Since $\ens' \in \hull(\{\ens[d], \ens_1\}) \subseteq \face(\{\ens[d], \ens_1\})$, every component of $\ens'$ is in $\face(\{\ens[d], \ens_1\})$ and therefore has $F$-value at most $f$.
		
		Since this holds for all $\ens' \in \hull(\{\ens[a], \ens_1, \ens_2\})$, we have $F(\component(\hull(\{\ens[a], \ens_1, \ens_2\}))) \subseteq (-\infty, f]$. Therefore $F(\face(\{\ens[a], \ens\})) \subseteq (-\infty, f]$ and $\ens \in S_f$.
	\end{proof}
\end{mathSection}
