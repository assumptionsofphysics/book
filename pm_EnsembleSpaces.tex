...
\begin{proof}
	...
	Let $\ens = \sum_j p_j \ens[a]_j \in \hull(\bigcap_i A_i)$ be a convex combination, possibly infinite, such that $\ens[a]_j \in \bigcap_i A_i$. This means that $\ens[a]_j \in A_i$ for all $i$ and all $j$. Since $\ens$ is a convex combination of elements of each $A_i$, $\ens \in \hull(A_i)$ for all $i$ and therefore $\ens \in \bigcap_i \hull(A_i)$. We have $\hull(\bigcap_i A_i) \subseteq \bigcap_i \hull(A_i)$. Conversely, let $\ens \in \bigcap_i \hull(A_i)$. Then we can write $\ens = \sum_j p_j \ens[a]_j$ where $\ens[a]_j \in \bigcap_i \hull(A_i)$. Therefore $\ens[a]_j \in \hull(A_i)$ for all $i$ and $j$. TODO: finish the proof.
\end{proof}
...
\begin{proof}
	...
	The previous argument generalizes to a family of convex subsets $\{A_i\}_{i \in I} \subseteq \mathfrak{Conv}_{\Ens}$. Moreover, the intersection of infinitely many closed sets is still closed. Therefore $\bigcap_{i \in I} A_i \in \mathfrak{Conv}_{\Ens}$ and $\mathfrak{Conv}_{\Ens}$ is an $\bigcap$-structure. This also means that it is a complete lattice.
\end{proof}
\end{mathSection}
