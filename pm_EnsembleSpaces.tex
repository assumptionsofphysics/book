...
\begin{prop}
	Let $C \subseteq \Ens$ be a classical probability context. The lattice of $\separate$-subspaces $\mathfrak{L}$ is commutative and is therefore a Boolean algebra.
\end{prop}

\begin{proof}
	The lattice of $\separate$-subspaces is a complemented bounded lattice. Note that a lattice is distributive if there are no sublattices \href{https://en.wikipedia.org/wiki/Distributive_lattice#Characteristic_properties}{ismorphic to $M_3$ and $N_5$}. Both require the existence of multiple complements. Therefore, if \href{https://en.wikipedia.org/wiki/Complemented_lattice#Orthocomplementation }{complements are unique} the lattice is distributive and is therefore a Boolean algebra.
	
	Let $A \in \mathfrak{L}$ be a subspace. A complement of $A$ is a subspace $B \in \mathfrak{L}$ such that $A \wedge B = \emptyset$ and $A \vee B = C$. Since $A \wedge B = \emptyset$, then $B \subseteq A^{\separate}$ (TODO: check or prove for all $R$-lattices). Since $A \vee B = C$, every element of $C$ is a mixture of elements of $A$ and $B$. Now suppose $\ens \in A^{\separate}$.
\end{proof}
...
Note that, in general, the lattice of $R$-subspaces is not a Boolean algebra. For example, the lattice of orthogonal subspaces is not distributive and therefore it is not a Boolean algebra, though it is a orthocomplemented lattice. The extra conditions of distributivity or, equivalently, the uniqueness of the lattice complements, are exactly what turn the orthocomplemented lattice into a Boolean algebra.
