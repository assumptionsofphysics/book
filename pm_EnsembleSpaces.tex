...
\begin{defn}
	An orthocomplemented lattice is orthomodular if $a \leq b$ implies $a \vee (a^{\ortho} \wedge b) = b$. An orthocomplemented lattice is orthoregular if 
\end{defn}

\begin{prop}
	Let $C \subseteq \Ens$ be a convex set where each decomposable element is separately (or orthogonally) decomposible. Then the lattice of $\mathcal{L}$ of $\separate$-subspaces (or $\ortho$-subspaces) is
\end{prop}
