...
\begin{proof}
	...
	4. Let $U, V \subseteq X$ such that $U^{RC} = (V^{RC})^{RC}$. Applying the component on each side, $(U^{RC})^{RC} = ((V^{RC})^{RC})^{RC}$. By the previous property $((V^{RC})^{RC})^{RC} = V^{RC}$ and therefore $(U^{RC})^{RC} = V^{RC}$. Switching $U$ and $V$ proves the other direction.
\end{proof}
...
\begin{conj}\label{pm_ensmblespaces_symirreflproperties}
	...
	\begin{enumerate}
		...
		\item $\bigcap_{i \in I} (U_i^{RC})^{RC} = ((\bigcap_{i \in I} (U_i^{RC})^{RC})^{RC})^{RC}$
	\end{enumerate}
\end{conj}
...
\begin{proof}
	...
	The set $\mathfrak{L}$ is a collections of subsets of $X$. Let $\{U_i\}_{i \in I} \subseteq \mathfrak{L}$ be a non-empty family. Then $\bigcap_{i \in I} U_i = \bigcap_{i \in I} (U_i^{RC})^{RC} = ((\bigcap_{i \in I} (U_i^{RC})^{RC})^{RC})^{RC} = ((\bigcap_{i \in I} U_i)^{RC})^{RC}$ by the third property of \ref{pm_ensmblespaces_symirreflproperties}. Therefore $\bigcap_{i \in I} U_i \in \mathfrak{L}$. This means $\mathfrak{L}$ is an $\bigcap$-structure. Using the second property of \ref{pm_ensmblespaces_symirreflproperties}, $X = \emptyset^{RC} = (X^{RC})^{RC}$. Therefore $\mathfrak{L}$ is a topped $\bigcap$-structure. This also means that it is a complete lattice.
\end{proof}
