...
\begin{figure}[h]
	...
	\end{tikzpicture}
	\caption{Visual representation of the fraction.}\label{pm-es-figFraction}
	%\includegraphics[width=0.6\textwidth]{tempimages/Fraction.jpg}
\end{figure}

First we define the \textbf{fraction} of one element with respect to another. For example, as shown in Fig. \ref{pm-es-figFraction}, suppose $\ens_{123456}$ is a uniform distribution for a six-faced die. Suppose $\ens_3$ represents the outcome 3 with $100\%$ probability. Then $\ens_{123456}$ can be understood as $\ens_{123456} = \frac{1}{6} \ens_3 + \frac{5}{6} \ens_{12456}$ where $\ens_{12456}$ is the uniform distribution over the outcomes 1,2,4,5 and 6. Note that $\frac{1}{6}$ is the highest coefficient we can put in front of $\ens_3$ in a convex combination and have $\ens_{123456}$ as a result, which coincides with the probability of obtaining 3 from a uniform distribution over six outcomes. That is how we define the fraction of $\ens_3$ with respect to $\ens_{123456}$.
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption{Discontinuity of fraction.}\label{pm-es-figFractionDiscontinuous}
\end{figure}

Intuitively, we would expect the fraction $\fraction_{\ens}(\ens[a])$ to be continuous both in $\ens[a]$ and $\ens$, but this is not the case. For example, as shown in Fig. \ref{pm-es-figFractionDiscontinuous}, let $\Ens$ be the Bloch ball and let $\ens[a]_i$ be a sequence of pure states (i.e. extreme points) that converge to another pure state $\ens$. Since they are all extreme points, we have $\fraction_{\ens}(\ens[a]_i) = 0$ and $\fraction_{\ens[a]_i}(\ens) = 0$ for all $i$. However, $\fraction_{\ens}(\ens) = 1$. This is the recurring problem of the non-continuity at the edges. The following conjecture, however, may still be true:
\begin{conj}
...
\begin{figure}[h]
	...
	\end{tikzpicture}
	\caption{Visual representation of the fraction capacity.}\label{pm-es-figFractionCapacity}
	%\includegraphics[width=0.6\textwidth]{tempimages/FractionCapacity.jpg}
\end{figure}

We now define \textbf{fraction capacity} of a set of ensembles with respect to another ensemble. Like before, as shown in Fig. \ref{pm-es-figFractionCapacity}, suppose $\ens_{123456}$ is a uniform distribution for a six-faced die, but now take $A = \{ \ens_3, \ens_4 \}$ as, respectively, the outcomes $3$ and $4$ with $100\%$ probability. Then $\ens$ can be understood as $\ens_{123456} = \frac{1}{3} \left(\frac{1}{2} \ens_3 + \frac{1}{2} \ens_4 \right) + \frac{2}{3} \ens_{1256}$, where $\ens_{1256}$ is the uniform distribution over outcomes 1,2,5 and 6. Again, note that $\frac{1}{3}$ is the highest coefficient we can put in front of any convex combination of elements of $A$, and still make a convex combination that has $\ens$ as a result. That is how we define the fraction capacity of $A$ with respect to $\ens_{123456}$.
...
\begin{figure}[H]
	...
	\caption{On the left, the cut triangle. On the right, the lattice of the orthogonal subspaces. The red lines connect the orthogonal complements.}\label{pm-es-figCutTriangleOrthoSubspaces}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}		
		...
		\draw (2) -- (b);
	\end{tikzpicture}
	\caption{Cut triangle.}\label{pm-es-figCutTriangle}
	%\includegraphics[width=0.4\textwidth]{tempimages/MultipleDecomposition.jpg}
\end{figure}
...
\subsubsection{Nicely boundaried sets}
