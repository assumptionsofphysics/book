...
\begin{remark}
	Note that if $A$ is not convex, this is clear not true. For example, let $A \subseteq \mathbb{R}^2$ ba an annulus (i.e. the region of two concentric circles). The points in the inner circle are internal points of $A$ according to the definition, but they are not interior points. An internal point of $A$ would be guaranteed to be surrounded by an open interval along any direction. The question, as usual, is if this is enough to fit an open set.
\end{remark}
