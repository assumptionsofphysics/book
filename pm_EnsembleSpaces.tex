	...
	\begin{proof}
		...
		\begin{figure}[H]
			...
			\begin{tikzpicture}
				...
				\end{axis}
				\node at (3,2) [above] {$\rho$};
				\node at (3.3,2) [above right] {$\rho+k$};
				\node at (1.1,1) [above left] {$\scriptstyle -\log_2(\rho)$};
				\node at (1.1,1) [below left] {$\scriptstyle -\log_2(\rho+k)$};
			\end{tikzpicture}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\coordinate (label1) at (6,3);
		\coordinate (label2) at (6,2);
		\coordinate (label3) at (6,1);
		\coordinate (a) at (2,2);
		\coordinate (b) at (4,3);
	...
	\caption{Once a reference point is picked (i.e. $\ens[a]$ in the figure), the differences with respect to that point behave like elements of a vector space. The relationship between ensembles in terms of mixtures become vector space relationships between differences.}\label{pm_es_vectorJustification}
\end{figure}
