...
\end{figure}
	...
	Let us show that the results also holds for to $\frac{1}{2} < p < 1$.
	Since $\{1 - \frac{1}{2^n}\}_{n=0}^{\infty}$ is a strictly decreasing sequence that starts at $1$ and converges to 0, there exists a unique $n \in \mathbb{N}$ such that $1 - \frac{1}{2^n} \le p \le 1 - \frac{1}{2^{n+1}}$. Let us show that $p\ens[a] + \bar{p}\ens[b]$ can be written as $\lambda\ens[a] + \bar{\lambda}[(1-\frac{1}{2^{n}})\ens[a] + \frac{1}{2^{n}}\ens[b]]$ where $0 \le \lambda \le \frac{1}{2}$.
	\begin{equation}
		\begin{aligned}
			p\ens[a] + \bar{p}\ens[b] 
	...
	Note that $0 \le \lambda \le \frac{1}{2}$ if $1 - \frac{1}{2^n} \le p \le 1 - \frac{1}{2^{n+1}}$. Thus, from the first part of the proof, we get $MS(p\ens[a], \bar{p} \ens[b]) \le MS(\ens[a], (1-\frac{1}{2^n}) \ens[a] + \frac{1}{2^n}\ens[b])$. Note that this result is recursive for $p = 1 - \frac{1}{2^n}$. Therefore, we can use induction to show $MS(p\ens[a], \bar{p} \ens[b]) \le MS(\ens[a], (1-\frac{1}{2^n}) \ens[a] + \frac{1}{2^n}\ens[b]) \le MS(\ens[a], \ens[b])$.
\end{proof}
\end{mathSection}
...
\begin{conj}
	Let $U \subseteq \Ens$ be an open set and $\ens[a] \in U$. Then there exists an $r \in (0,1]$ such that $B_r(\ens[a]) \subseteq U$.
\end{conj}

\begin{remark}
	These conjectures are likely the same. If the topology is generated by the entropic open balls, then convergence in mixing entropy is the convergence criterion associated with the topology. It also means that we can generate every open set with entropic open balls by fitting them inside every open set.
