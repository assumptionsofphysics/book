...
\subsection{Mixing probability}

\begin{defn}
	Let $\ens \in \Ens$. The \textbf{mixing probability measure} for $\ens$ is defined as $p_{\ens}(A) = \sup(\{ p \in [0,1] \, | \, \exists \, \ens_1 \in \hull(A), \ens_2 \in \Ens \text{ s.t. }  \ens = p \ens_1 + \bar{p} \ens_2 \})$ if $A \neq \emptyset$ and $p_{\ens}(A) = 0$ otherwise.
\end{defn}
...
\begin{coro}
	The mixing probability measure uniquely identify an ensemble. That is, let $\ens_1, \ens_2 \in \Ens$ such that $\ens_1 \neq \ens_2$. Then $p_{\ens_1} \neq p_{\ens_2}$.
\end{coro}
...
\begin{prop}
	The mixing probability measure for an ensemble is
	\begin{enumerate}
...
\begin{conj}
	Let $\{U^i\}_{i=1}^{n} \subseteq \Ens$ be a family of sets of ensembles such that $\ens \in \hull(\bigcup U^i)$. Then $p_{\ens}(\bigcup U^i) = \sum p_{\ens}(U_i)$ if and only if we can find a maximal component sequence within each set such that its elements are distinct from those of any other sequence.
\end{conj}
...
\begin{proof}
	TODO: the closest expression for the QFIM we found is in \href{https://link.springer.com/chapter/10.1007/978-4-431-54493-7_4}{Quantum Estimation Theory}. The RLD Fisher information is $\tr(\rho L_i L_j)$ where $L_i$ is the right logarithmic derivative. We cab re-express $\frac{1}{\rho} \delta \rho^2 = \rho \frac{\delta\rho}{\rho} \frac{\delta\rho}{\rho}$ given that $\partial_i \rho = \rho L_i$.
\end{proof}
