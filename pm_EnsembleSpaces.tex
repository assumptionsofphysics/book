...
\begin{conj}
	...
	\begin{enumerate}
		...
		\item the lattice complements are unique (?)
	\end{enumerate}
\end{conj}
...
\begin{proof}
	...
	Since \href{https://proofwiki.org/wiki/Complement_in_Distributive_Lattice_is_Unique}{complements in a distributive lattice are unique}, 2 implies 3. Since \href{https://en.wikipedia.org/wiki/Complemented_lattice#Orthocomplementation}{orthocomplemented lattices satisfy de Morgan's laws} and  \href{https://proofwiki.org/wiki/De_Morgan%27s_Laws_imply_Uniquely_Complemented_Lattice_is_Boolean_Lattice}{de Morgan laws imply distributivity for a uniquely complemented lattice}, 3 implies 2.
\end{proof}
...
%\begin{defn}[Ensemble equation]
%	Let $\{\ens[a]_i\}_{i=1}^n, \{\ens[b]_i\}_{j=1}^m \subseteq \Ens$ be two finite sequences of ensembles and $\{r_i\}_{i=1}^n, \{s_i\}_{j=1}^m \subseteq \mathbb{R}$ be two finite sequences of coefficients such that $\sum_{i=1}^n r_i = \sum_{j=1}^m s_j = t$. The \textbf{ensemble equation} $\sum_{i=1}^n r_i \ens[a]_i = \sum_{j=1}^m s_j \ens[b]_j $ is satisfied if .
%\end{defn}
%
%\begin{remark}
...
\begin{defn}
	Two ensemble differences are \textbf{equivalent}, noted $r(\ens[b] - \ens[a]) \sim s(\ens[c] - \ens[a])$, if
	\begin{itemize}
		\item $r=0$ and $s=0$
		\item $r=-s$ and $\frac{1}{2} \ens[b] + \frac{1}{2} \ens[c] = \ens[a]$ or
		\item $\frac{r}{r+s}\ens[b] + \frac{s}{r+s} \ens[a] = \frac{s}{r+s} \ens[c] + \frac{r}{r+s} \ens[a]$.
	\end{itemize}
\end{defn}

\begin{remark}
	Note that the last condition is an equation between affine combinations are the coefficients can be negative. The equality can be satisfies only if both affine combinations exist.
\end{remark}

\begin{coro}
	We have $r(\ens[b] - \ens[a]) \sim 0(\ens[c] - \ens[a])$ if and only if either $r=0$ or $\ens[b] = \ens[a]$.
\end{coro}

\begin{proof}
	If $r=0$, then the two differences are equivalent. If $\ens[b] = \ens[a]$, we have $\frac{r}{r}\ens[a] + \frac{0}{r} \ens[a] = \frac{0}{r} \ens[c] + \frac{r}{r} \ens[a]$ which means the differences are equivalent.
	
	Conversely, suppose the two differences are equivalent. The first and second conditions are satisfied only if $r=0$. For the third, we have $\frac{r}{r}\ens[b] + \frac{0}{r} \ens[a] = \frac{0}{r} \ens[c] + \frac{r}{r} \ens[a]$ which is satisfied only if $\ens[b] = \ens[a]$.
\end{proof}
...
\begin{proof}
	For reflexivity, we have $\frac{r}{2r}\ens[b] + \frac{r}{2r} \ens[a] = \frac{r}{2r} \ens[b] + \frac{r}{2r} \ens[a]$, which is satisfied for any $\ens[a], \ens[b] \in \Ens$ and $r \in \mathbb{R}$.
	
	For symmetry, note that all three conditions remain the same if $r$ is switched with $s$ and $\ens[b]$ with $\ens[c]$.
	
	For transitivity, suppose $r(\ens[b] - \ens[a]) \sim s(\ens[c] - \ens[a])$ and $s(\ens[c] - \ens[a]) \sim t(\ens[d] - \ens[a])$. Since there are different condition for equivalence, we have to check all combinations. However, since symmetry holds, the order in which the different conditions apply does not matter.
	
	If both equivalences hold for the first condition, then all coefficients are zero and $r(\ens[b] - \ens[a]) \sim t(\ens[d] - \ens[a])$ for the first condition.

	If the first equivalence holds for the first condition and the second equivalence holds for the second condition, we still have that all coefficients are zero.

	If the first equivalence holds for the first condition and the second equivalence holds for the third condition, then $s \neq 0$ and $\ens[c] = \ens[a]$. We have $\frac{s}{s+t}\ens[a] + \frac{t}{s+t} \ens[a] = \frac{t}{s+t} \ens[d] + \frac{s}{s+t} \ens[a]$ which means $\ens[d] = \ens[a]$ and therefore $r(\ens[b] - \ens[a]) \sim t(\ens[d] - \ens[a])$.
	
	If both equivalences hold for the second condition, then $r=-s=t$ and $\frac{1}{2} \ens[b] + \frac{1}{2} \ens[c] = \ens[a] = \frac{1}{2} \ens[c] + \frac{1}{2} \ens[d]$. This means that $\ens[b] = \ens[d]$ and therefore $r(\ens[b] - \ens[a]) = t(\ens[d] - \ens[a])$.

	If the first equivalence holds for the second condition and the second equivalence holds for the third condition, we have $r=-s$ and $\frac{1}{2} \ens[b] + \frac{1}{2} \ens[c] = \ens[a]$, which means $\ens[b] = 2 \ens[a] - \ens[c]$. We can re-express $\frac{s}{s+t}\ens[c] + \frac{t}{s+t} \ens[a] = \frac{t}{s+t} \ens[d] + \frac{s}{s+t} \ens[a]$ as $\ens[c] = \frac{s-t}{s} \ens[a] +\frac{t}{s}\ens[d]$. Putting it together $\ens[b] = 2 \ens[a] - \left(\frac{s-t}{s} \ens[a] +\frac{t}{s}\ens[d]\right) =\frac{2s - s +t}{s} \ens[a] - \frac{t}{s}\ens[d] = \frac{s+t}{s} \ens[a] - \frac{t}{s}\ens[d] = \frac{s+t}{s} \ens[a] - \frac{t}{s}\ens[d] = \frac{r-t}{r} \ens[a] + \frac{t}{r}\ens[d]$. Which means $\frac{r}{r+t}\ens[b] + \frac{t}{r+t} \ens[a] = \frac{t}{r+t} \ens[d] + \frac{r}{r+t} \ens[a]$. Therefore $r(\ens[b] - \ens[a]) \sim t(\ens[d] - \ens[a])$.

	If both equivalences hold in both cases for the third condition we have $\ens[b] = \frac{r-s}{r} \ens[a] +\frac{s}{r}\ens[c]$ and $\ens[c] = \frac{s-t}{s} \ens[a] +\frac{t}{s}\ens[d]$. Putting them together, $\ens[b] = \frac{r-s}{r} \ens[a] +\frac{s}{r}\left(\frac{s-t}{s} \ens[a] +\frac{t}{s}\ens[d]\right) = \frac{r-s + s -t}{r} \ens[a] +\frac{t}{r}\ens[d] = \frac{r-t}{r} \ens[a] +\frac{t}{r}\ens[d]$. Which means $\frac{r}{r+t}\ens[b] + \frac{t}{r+t} \ens[a] = \frac{t}{r+t} \ens[d] + \frac{r}{r+t} \ens[a]$. Therefore $r(\ens[b] - \ens[a]) \sim t(\ens[d] - \ens[a])$.
\end{proof}

\begin{coro}
	The equivalence relationship satisfies the following:
	\begin{enumerate}
		\item the equivalence class $\left[0(\ens[b] - \ens[a])\right]$ is given by the set $\left\{0\left(\ens - \ens[a]\right)\right\} \cup \left\{r\left(\ens[a] - \ens[a]\right)\right\}$ for all $\ens \in \Ens$ and $r \in \mathbb{R}$
		\item for $r \neq 0$, the equivalence class $\left[r(\ens[b] - \ens[a])\right]$ is given by the set $\left\{s\left(\frac{r}{s}\ens[b] + \frac{s-r}{s}\ens[a] - \ens[a]\right)\right\}$
		...
		\item given $r,s \in \mathbb{R} \setminus \{0\}$ and $\ens[b] \neq \ens[a]$, $r(\ens[b] - \ens[a]) \sim s(\ens[b] - \ens[a])$ if and only if $r=s$
		\item given $r,s \in \mathbb{R} \setminus \{0\}$ with $r\neq s$, $r(\ens[b] - \ens[a]) \sim s(\ens[b] - \ens[a])$ if and only if $\ens[b] = \ens[a]$.
	\end{enumerate}
\end{coro}

\begin{proof}
	For 1, Note that $r(\ens[b] - \ens[a]) \sim 0(\ens[c] - \ens[a])$ if and only if either $r=0$ or $\ens[b] = \ens[a]$.
	
	For 2, note that third condition can be reexpressed as $\ens[c] = \frac{s-r}{r} \ens[a] +\frac{r}{s}\ens[b]$. If $r=-s$, the same expression recovers the second condition.
	
	For 3, since 2 tells us that $\ens[c]$ is an affine combination of $\ens[a]$ and $\ens[b]$, it must be on the same line.
	
	For 4, if $r\neq0$ just substitute $s= \frac{r}{p}$ in 2. If $r=0$, then $\frac{r}{p}=0$ and therefore the two differences are equivalent.
