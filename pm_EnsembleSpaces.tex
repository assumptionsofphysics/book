...
\begin{defn}
	A convex set $V \subseteq \Ens$ is finite dimensional if it embeds into a finite dimensional subspace of the embedding vector space. A finite dimensional convex set $V$ is a manifold, and is endowed with a differentiable structure given by all coordinates that are infinitely smooth with respect to the mixing coefficients (i.e. to the affine structure). A \textbf{linear chart} is a chart that preserves the linearity of the affine structure. A \textbf{variation} $\delta \ens$ is an element of the tangent space expressed in a linear chart.
\end{defn}
...
\begin{proof}
	To recover the first two expressions, we simply have to calculate the leading term. Since the entropy is twice differentiable, assuming a linear chart, we can expand it as
	\begin{equation}
		\begin{aligned}
	...
	Let us now show that $g$ is a metric tensor. Restricted to $V$, $g|_V$ is minus the Hessian of the entropy $S$ in a linear chart. Therefore it is a symmetric rank-two tensor which means a symmetric bilinear function of the arguments. Since the entropy is strictly concave, the Hessian is negative definite and therefore $g$ is positive definite. Therefore $(V, g|_V)$ is a Riemannian manifold.
\end{proof}
\end{mathSection}
...
\begin{conj}
	A flat $C \subseteq \Ens$ is a classical probability context if and only if mixtures preserve separateness in $C$. That is, if $\ens \separate \ens[a]$ and $\ens \separate \ens[b]$, then $\ens \separate p\ens[a] + \bar{p} \ens[b]$ in $C$ for all $\ens, \ens[a], \ens[b] \in C$ and $p \in [0,1]$.
\end{conj}
...
\subsubsection{Classical probability contexts in classical and quantum ensemble spaces}
