...
\begin{mathSection}
	...
	\begin{proof}
		...
		For 1, since the lattice is an $\bigcap$-structure, $A \wedge A^{R} = A \cap A^{R}$ and from \ref{pm_ensmblespaces_symirreflproperties} we have $A \cap A^{R} = \emptyset$. Since the lattice is an $\bigcap$-structure, $A \vee A^{R} \supseteq A \cup A^{R}$. Using \ref{pm-es-rComplProps} and \ref{pm_ensmblespaces_symirreflproperties} we have $((A \vee A^{R})^{R})^{R} \supseteq ((A \cup A^{R})^{R})^{R} = (\emptyset)^{R} = X$.
	\end{proof}
