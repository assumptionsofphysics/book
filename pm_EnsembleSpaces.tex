...
\begin{mathSection}
	...
	\end{proof}
\end{mathSection}
This is not, however, enough to show that $\Ens$ is bounded. A set, in fact, could be bounded in every direction but still be unbounded as there is always a direction with a greater bound. That is, directional boundedness is a pointwise one-dimensional property, while boundedness around a point is a uniform property over all directions. Therefore, the question as to whether all ensemble spaces are bounded is open.

\begin{conj}
	Every ensemble space $\Ens$ is bounded around every point.
\end{conj}
