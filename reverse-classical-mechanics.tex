...
In the previous sections, we saw that elements of differential geometry started appearing: we employed a generalization of curl and we used the form $\omega_{ab}$ to lower indexes, much like one does in general relativity with the metric tensor $g_{ab}$. Since we will use these tools more and more, we should take a detour and understand the physical significance of the tools themselves. We will see that to reach the full potential of reverse physics, we need to apply the same techniques to not just the equations themselves, but to the mathematical tools we use to formulate them.

The main reason we are forced to abandon vector calculus in favor of differential topology and geometry is that vector calculus does not generalize to an arbitrary dimensional space. Special and general relativity, for example, live on a four-dimensional space-time; Hamiltonian and Lagrangian mechanics live in phase-space, which is even larger as we can have an arbitrarily large number of degrees of freedom. Given that our goal is ultimately physics, we will start from the expressions of vector calculus, see their limitations, and construct generalized ones. We warn the reader that already is familiar with differential topology that what we will obtain is not exactly the notation and concepts used by mathematicians. We will discuss this difference at the end of the section.

The first tool we need go generalize is that of line, surface and volume integral. For example, the mass $m$ within a region $V$ can be understood as the sum of the contributions of a mass density $\rho$ within each infinitesimal volume $dV$:
\begin{equation}
	m(V) = \iiint_V \rho dV.
\end{equation}
...
\end{equation*}
Lastly, the work $W$ over a path $\lambda$ can be understood as the sum of the contributions of a force field $\vec{f}$ over each infinitesimal line $\vec{d\gamma}$:
\begin{equation*}
	W(\gamma) = \int_\gamma \vec{f} \cdot \vec{d\gamma}.
\end{equation*}

Note the pattern: the functionals $W$, $\Phi$ and $m$ all take a region of space while $\vec{f}$, $\vec{B}$ and $\rho$ act on infinitesimal regions. However the pattern is not completely consistent. In the one dimensional case, we have the product between a vector representing the force and a vector representing the displacement along the line. In the two dimensional case we the product between a pseudo-vector representing the magnetic force and a vector representing the normal to the surface element. In the three dimensional case we have the product between a pseudo-scalar representing the density and a scalar representing the volume of the infinitesimal region. Note that each operation is ad-hoc. Moreover, a surface has a single perpendicular direction only in two dimensions. Lastly, they all require a notion of product between vector, an inner product, which is only defined on Riemannian spaces, those that define a metric tensor. That is, those spaces in which angles and distances are well defined. In physics we are so used to work with spaces that have an inner product that it may seem that all spaces provide one, but that is not the case. In phase space, there is no way to compare differences in position with differences in momentum, therefore we do not have a metric to take a scalar product. In general, if we imagine the space that represents all possible outcomes of a blood test, this will form a manifold as each test can be fully described by a finite set of continuous quantities. In this space, there is no natural notion of distance and angles between directions.

Suppose we understood $f$, $B$ and $\rho$ not as above, but as functions of infinitesimal regions. We could simply write
\begin{equation*}
	\begin{aligned}
	...
	\end{aligned}
\end{equation*}
This pattern is more straight-forward, and clearly generalized. We call these functions of infinitesimal regions $k$-form, where $k$ is the dimensionality of the infinitesimal region they take as an argument. The force, in this notation, is a one-form (or covector) as it takes one dimensional infinitesimal regions (i.e. vectors); the magnetic field is a two-form; the density is a three-form. We can also say that a scalar field, like the temperature, is a zero-form.

Given that the $k$-form act over infinitesimal regions, they can be further described. First, they will be linear functions of the infinitesimal regions. Second, note that each infinitesimal region can be understood as a parallelepiped, and a parallelepiped is fully identified by its sides. Therefore, a $k$-form can be understood as acting on a set of infinitesimal displacements, the sides of the parallelepiped, that matches the dimensionality of the form. A one-form will take one displacement, a two-form two displacements and so on. Lastly, all forms must be anti-symmetric because switching the order of the sides does not change the parallelepiped, but it changes its orientation.

We can write displacements and forms in terms of components and basis 
\begin{equation}
	\begin{aligned}
	dP &= dx^i e_i \\
		f &= f_i e^i \\
		B &= B_{ij} e^i \otimes e^i \\
		\rho &= \rho_{ijk} e^i \otimes e^j \otimes e^k.
	\end{aligned}
\end{equation}
Therefore, our integrals can be written as
\begin{equation}
	\begin{aligned}
	...
	\end{aligned}
\end{equation}
Each form, then, is a fully anti-symmetric covariant tensor. The $B^x$ component of the magnetic field, then, becomes the $B_{yz}$ component. That is, instead of being the component that gives us the magnetic flux along the $x$ direction, it is the component gives us the flux along the $yz$ plane. The function $\rho_{xyz}$ is not the value of the mass density at a point, but rather the value of the density over the $xyz$ volume.

With that in mind, we can rewrite the integrals in terms of parametrizations of the surfaces and the components of the forms along said parametrization. We have
\begin{equation}
	\begin{aligned}
	...
	\end{aligned}
\end{equation}
This expressions are useful to write the integrals in terms of terms of a single integral of $k$ variables.

We now have expressions that are easier to generalize. If we note $S^k$ the space of all the $k$ dimensional subregions, a $k$-functional $F_f : S^k \to \mathbb{R}$ is an additive functional that takes a $k$-surface $\sigma^k$ and returns a number. This can be expressed as
\begin{equation}
	\begin{aligned}
		F_k(\sigma^k) &= \int_\sigma \omega_k (d\sigma^k) = \int_{\sigma^k} \omega_{i_1 i_2 \cdots i_k} \, dx_1^{i_1} \, dx_2^{i_2} \, \cdots \, dx_k^{i_k} \\
		&= \int_{a_{u_1}}^{b_{u_1}} \int_{a_{u_2}}^{b_{u_2}} \cdots \int_{a_{u_k}}^{b_{u_k}} \omega_{u_1 u_2 \cdots u_k} \, du_1 \, du_2 \, \cdots \, du_k.
	\end{aligned}
\end{equation}
the integral of a $k$-form $\omega_k : V^k \to \mathbb{R}$ that takes an infinitesimal $k$-surface $d\sigma^k$, defined by a set of $k$ vectors, and returns a number. We now have a way to express local objects and integrals in a generalized way.

In the spirit of reverse physics, we ask: what are the physical assumptions that are required to use these mathematical objects? Note that we do not measure directly mass density $\rho$: we measure mass $m$ within a finite region $V$, the size of the finite region $V$, and we calculate the mass density by dividing the first with the second. The mass $\rho$  is the limit for which the region $V$ shrinks to a point. Same with the other quantities. We do not measure a force $f$ directly, but rather the work that force performs, for example, by deforming a spring. In other words, we measure the finite value of the functional $F$ over a finite region $\sigma$, and the form $\omega$ returns the limit of $F$ for infinitesimal regions. Note that it is crucial for $F$ to be additive, that is, if $\sigma_1$ and $\sigma_2$ are two disjoint regions, $F(\sigma_1 \cup \sigma_2) = F(\sigma_1) + F(\sigma_2)$. Physically, it means that the contribution of each sub-region is independent from the other, because if this isn't the case, we cannot assign a unique value to each region. While they may seem a relatively harmless assumption, it may not hold in general. For example, suppose we have a system distributed in space. Given the mass/energy equivalence, the mass is an additive functional only if the interaction between the parts can be neglected. In fact, if the interaction energy at the boundary is so large that it is of the same scale of the mass within the regions, then it is not true that the total mass is simply the mass within each region. Therefore, if we write the mass density $\rho$, we have implicitly assumed that the interaction energy between parts can be neglected. While this is going to be true in a large number of cases, it is something we have to keep in mind. The functional $F$, in fact, is a more general physical object as it may exists even though infinitesimal additivity fails.

If the functional $F$ respects the limits, in the sense that small variations of the region results in small variations of the value of the functional, then we can express the functional $F$ as the integral of a form $\omega$.\footnote{Mathematically, one needs to be precise as to what these small variations are, and what is the space of regions. Effectively, we need to define what it means for $k$-surfaces to be differentials. We leave this for another section.} Note that the standard mathematical definitions are inverted to what makes physical sense. Mathematically, we first define the forms and then its integration, and we may worry whether the integral exists or diverges; physically, we first define the functional of finite regions and find the form as a limit on additional physical assumptions. This is important because, as we said before, it tells us that it is the functional that survives when the assumption fails, not the form. It also means that, if we want to get a robust physical intuition, we should concentrate on the functional, the finite object, rather than the forms, the infinitesimal objects.

As we saw before, we also need to generalize the differential operators gradient, curl and divergence. Suppose that $T$ is a scalar field, like the temperature. Then we have the following relationship
...
\begin{equation}\label{rp-cm-dg-curlTheorem}
	\int_{\Sigma} \vec{\nabla} \times f \cdot d\vec{\Sigma} = \int_{\lambda = \partial \Sigma} \vec{f} \cdot d\vec{\lambda} = W(\lambda)
\end{equation}
...
\begin{equation}\label{rp-cm-dg-generalizedStokes}
	\begin{aligned}
		\partial F_k(\sigma^{k+i}) &=\int_{\sigma^{k+i}} \partial_{i_0} \wedge \omega_{i_1 i_2 \cdots i_k} dx^{i_0} dx^{i_1} dx^{i_2} \cdots dx^{i_k} \\
		&= \int_{\partial \sigma^{k+i}} \omega_{i_1 i_2 \cdots i_k} dx^{i_1} dx^{i_2} \cdots dx^{i_k} = F_k(\partial \sigma^{k+i}).
	\end{aligned}
\end{equation}
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
What we can see is that, in each expression, we are taking the derivative with respect to each index. The pattern generalizes in the following way
\begin{equation}
	\begin{aligned}
		\partial_{i_0} \wedge \omega_{i_1 i_2 \cdots i_k} &= \partial_{i_0} \omega_{i_1 i_2 \cdots i_k} + (-1)^{k} \partial_{i_1} \omega_{i_2 \cdots i_k i_0} + (-1)^{2k} \partial_{i_2} \omega_{i_3 \cdots i_k i_0 i_1} + \cdots \\
		&+ (-1)^{k^2} \partial_{i_k} \omega_{i_0 i_1 \cdots i_{k-1}} \\
		&= \sum_{j=0}^{j=k} (-1)^{jk} \partial_{i_{j \bmod k}} \omega_{i_{j+1 \bmod k} i_{j+2 \bmod k} \cdots i_{j+k \bmod k}}.
	\end{aligned}
\end{equation}

Mathematically, we can understand the expression as the requirement of the resulting form to be anti-symmetric. Since all permutations must be accounted for, and since the form will already take into account the permutations among its indexes, we need one term for each derivation in each index, making sure to use the correct orientation. We can do that in different way, but here we use of cyclic permutations to . Geometrically, we can imagine integrating along a parallelepiped, which becomes
\begingroup
\allowdisplaybreaks
\begin{align*}
		\int_{\sigma^{k+1}} \partial \wedge \omega_k (d\sigma^{k+1}) &= \int_{\sigma^{k+1}} \partial_{i_0} \wedge \omega_{i_1 i_2 \cdots i_k} \, dx_1^{i_1} \, dx_2^{i_2} \, \cdots \, dx_k^{i_k} \\
		&= \int_{a_{u_0}}^{b_{u_0}} \int_{a_{u_1}}^{b_{u_1}} \cdots \int_{a_{u_k}}^{b_{u_k}} (\partial_{u_0} \omega_{u_1 u_2 \cdots u_k} \\
		&+ (-1)^{k} \partial_{u_1} \omega_{u_2 \cdots u_k u_0} + \cdots 
		...
		&+ (-1)^{k} \left[ \int_{a_{u_0}}^{b_{u_0}} \int_{a_{u_2}}^{b_{u_2}} \cdots \int_{a_{u_k}}^{b_{u_k}}\omega_{u_1 u_2 \cdots u_k} \,du_0 \, du_2 \, \cdots \, du_k \right]_{a_{u_1}}^{b_{u_1}} \\
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
In other words, the exterior derivative applied twice returns zero. To understand the geometrical significance of the generalized relationship, recall that the exterior derivative of the form is associated with the exterior functional. We should, then, look at what happens when we construct the exterior functional of an exterior functional. We have
\begin{equation}
	\begin{aligned}
		\partial \partial F_k (\sigma^{k+2}) &= \partial F_k (\partial \sigma^{k+2}) = F_k (\partial \partial \sigma^{k+2}) = F_k(\emptyset) = 0.
	\end{aligned}
\end{equation}
In words, the exterior of the exterior functional of $F_k$ equals $F_k$ applied to the boundary of the boundary. Now, the boundary of a boundary is always the empty set, and any functional applied to the empty set must be zero. In fact, since functionals are additive, we must have
\begin{equation}
	\begin{aligned}
		F_k(\sigma_k) &= F_k(\sigma_k \cup \emptyset) = F_k(\sigma_k) + F_k(\emptyset).
	\end{aligned}
\end{equation}
This shows how studying differential relationship in terms of finite functionals can give a more geometrically meaningful picture.
...
Technically, closed forms (i.e. those whose exterior derivative is zero) are not necessarily exact forms (i.e. those that are the exterior derivative of another form). This is true only on contractible regions (i.e. those regions that can be continuously shrunk to a point, that do not have holes). We can understand the difference better by looking at the corresponding functionals. An exact form corresponds to a functional that returns zero for any closed surface. A closed form, however, is guaranteed to return zero only on closed surfaces that are contractible. For example, it may return non-zero over a closed surface that encloses a hole. Therefore, all exact forms are closed, but not all closed forms are exact. However, if we restrict ourselves to a contractible region, the two definitions are the same.
...
\end{equation}
the form $\omega_{ab}$ is the curl of $- \theta_a$ generalized to $n$ dimensions. In this sense, $\theta_a$ can be understood as a vector potential of $\omega$.

Given that the extended phase space for a single degree of freedom is three dimensional, this case can be understood with standard vector calculus. In this case, we have
\begin{equation}
	\begin{aligned}
	\theta_a &= \begin{bmatrix}
		p & 0 & - H
...
\end{bmatrix} .
\end{equation}
Vector potentials are defined up to the gradient of an arbitrary function, up to a gauge, and therefore we can set, without loss of generality, the momentum component to zero
\begin{equation}
	\theta_a = \begin{bmatrix}
		\theta_q & 0 & \theta_t
	\end{bmatrix} .
\end{equation}
Now, the displacement field $S^a$ is particular in that its time component $S^t = d_t t = 1$ is unitary. That is, states flow at the same rate in time. Therefore we must have
\begin{equation}
...
This argument tells us very simply what happens in the case of a single degree of freedom, but, as we have seen before, the divergenceless of $S^a$ is not enough to characterize Hamiltonian mechanics in the general case. It is instructive to understand how it generalizes, to see that it recover only part of the picture. To do that we are now forced to introduce a couple of basic elements of differential topology because we really need to understand how gradient, curl and divergence generalize.\footnote{We are going to use concepts and notation that is more straightforward for those with a physics background. The general idea is that vectors represent linear displacements and infinitesimal regions. Tensors, and forms in particular, are linear functions of displacements and infinitesimal regions.}

Suppose that $T$ is a scalar field, like the temperature. Then we have the following relationship
\begin{equation}
	\int_{\lambda} \vec{\nabla} T \cdot d\vec{\lambda} = T(B) - T(A).
\end{equation}
That is, the integral of the gradient of $T$ along $\lambda$ gives us the difference of $F$ evaluated at the endpoints, the boundary of the line. If $\vec{F}$ is a vector field, like the force, we have
\begin{equation}
	\int_{\Sigma} \vec{\nabla} \times F \cdot d\vec{\Sigma} = \int_{\lambda = \partial \Sigma} \vec{F} \cdot d\vec{\lambda}
\end{equation}
That is, the integral of the curl of $\vec{F}$ over a surface $\Sigma$ is equal to the line integral of $\vec{F}$ over the boundary. If $\vec{B}$ is a vector field, like the magnetic field, we have
\begin{equation}
	\int_{V} \vec{\nabla} \cdot \vec{B} \, dV = \int_{\Sigma = \partial V} \vec{B} \cdot d\vec{\Sigma}.
\end{equation}
That is, the integral of the divergence of $\vec{B}$ over a region $V$ is equal to the surface integral of $\vec{B}$ over the boundary.

Note the pattern: the integral of the differential operator applied to the bulk becomes the integral of the original object over the boundary. This relationship is what differential geometry generalizes. The trick is to understand that $\vec{B}$ is not really a vector, it is a pseudo-vector. It acts differently under parity precisely because we integrate it over surfaces, not lines. Therefore, instead of thinking it as a vector $B^i$ we can more properly characterize it as an anti-symmetric tensor $B_{ij}$. In this form, the pattern of the expressions becomes more streamlined
\begin{equation}
	\begin{aligned}
		T(P) & \\
		W(\lambda) &= \int_\lambda F_i dx^i = \int_{u_1}^{u_2} F_i \partial_u x^i du = \int_{u_1}^{u_2} F_u du \\
		\Phi(\Sigma) &= \int_\Sigma B_{ij} dx^i dx^j = \int_{u_1}^{u_2} \int_{v_1}^{v_2} B_{ij} \partial_u x^i \partial_v x^j du dv = \int_{u_1}^{u_2} \int_{v_1}^{v_2} B_{uv} du dv.
	\end{aligned}
\end{equation}
Note that we give the expression both in coordinates $x^i$ of the manifold, and the coordinates of the $k$-surface for integration. These can be generalized to a generic $k$-dimensional surface $\sigma_k$ and a generic tensor $\theta_{i_1 i_2 \cdots i_k}$
\begin{equation}
	F(\sigma_k) = \int_{\sigma_k} \theta_{i_1 i_2 \cdots i_k} dx^{i_1} dx^{i_2} \cdots dx^{i_k} = \int_{u_1}^{u_2}\int_{v_1}^{v_2}\cdots\int_{w_1}^{w_2} \theta_{u v \cdots w} du dv \cdots dw.
\end{equation}
These tensors are called forms, and they must be fully anti-symmetric to match the transformation rules of the $k$-dimensional parallelepipeds they act on.

Note that the gradient, the curl and the divergence always increase the region of integration by one dimension. This is because they take forms that act on the boundary, and return forms that act in the bulk. The idea is that we can construct an operator called exterior derivative, which we will note $\nabla_{i_0} \theta_{i_1 i_2 \cdots i_k}$ such that
\begin{equation}
	\int_{\sigma_{k+i}} \nabla_{i_0} \theta_{i_1 i_2 \cdots i_k} dx^{i_0} dx^{i_1} dx^{i_2} \cdots dx^{i_k} = \int_{\partial \sigma_{k+i}} \theta_{i_1 i_2 \cdots i_k} dx^{i_1} dx^{i_2} \cdots dx^{i_k}.
\end{equation}
The expression in terms of terms of components works in the following way
\begin{equation}
	\begin{aligned}
		\nabla_i T &= \partial_i T \\
		\nabla_i F_j &= \partial_i F_j - \partial_j F_i \\
		\nabla_i B_{jk} &= \partial_{i} B_{jk} + \partial_{j} B_{ki} + \partial_{k} B_{ij} \\
		\nabla_i \theta_{jkl} &= \partial_{i} \theta_{jkl} - \partial_{j} \theta_{kli} + \partial_{k} \theta_{lij} - \partial_{l} \theta_{ijk} \\
	\end{aligned}
\end{equation}
Note how we have a derivative in each index in each direction, and how the exterior derivative applied to odd dimensional forms (i.e. one and three) have alternating signs. The alternating sign is a consequence of the anti-symmetry of the form.

The other result is that the exterior derivative applied twice is always zero, that is $\nabla_i \nabla_j \theta_{k\cdots} = 0$. This generalizes the fact that the curl of the gradient is zero and that the divergence of the curl is zero. And that if the exterior derivative of a form is zero, then it can be expressed as the exterior derivative of a lower dimensional form. Therefore, if the exterior derivative of a four-form is zero $\nabla_i \omega_{jklm} = 0$, then it can be there is a three-form potential $\omega_{jklm} = \nabla_j \theta_{klm}$. This generalizes the idea that a curl-free field has a scalar potential or that a divergence-free field (which is more properly a two-form) has a vector potential (which is more properly a one-form).

The details of the math are not important here, but the concepts they represent. The three key ideas are
\begin{enumerate}
	\item $k$-forms are objects that are integrated over a $k$-dimensional surface and return a number
	\item the exterior derivative $\nabla_i$ takes a $k$-form and returns a $k+1$-form that applies the original form on the boundary
	\item if a $k$-form has a zero exterior derivative, then there is a $(k-1)$-form, a potential, such that the $k$-form is the exterior derivative of the $(k-1)$-form. 
\end{enumerate}
