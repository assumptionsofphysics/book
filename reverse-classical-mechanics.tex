...
To simplify our discussion, we will concentrate on a single degree of freedom. The first characterization of Hamiltonian mechanics is naturally in terms of the equations
\begin{equation}
	\begin{aligned}
		d_t q &= \partial_p H \\
		d_t p &= - \partial_q H.
...
\begin{equation}\label{rp-cm-symplectic1d}
	...
	\end{array} \right]
\end{equation}
which rotates a vector by a right angle.\footnote{The notion of angle is technically ill-defined in phase space, but this slight imprecision makes it easier to get the point across.} That is, if $v^a = [v^q, v^p]$, then $\omega_{ab} v^a = [v^p, - v^q]$. We can rewrite equation \ref{rp-cm-displacementCurl1d} as
\begin{equation}\label{rp-cm-displacementCurlS1d}
	\begin{aligned}
		\omega_{ab} S^b &= \partial_a H 
	\end{aligned}
\end{equation}
...
\end{equation}
Geometrically, this means that the flow of $S^a$ through a closed region $\oint \left( S^q dp - S^p dq \right) = 0$ is always zero. Note that, since we are in a two dimensional space, a hyper-surface has dimension $n-1 = 2-1 = 1$ and therefore hyper-surfaces are lines. The integral through a boundary is effectively a line integral along the boundary with the field rotated 90 degrees. Therefore we have 
\begin{equation}
	\oint \left( S^p dq - S^q dp \right) = \oint \omega_{ab} S^b d\xi^a = \oint \partial_a H d\xi^a = \oint dH = 0.
\end{equation}
That is, the line integral of the rotated displacement field is the line integral of the gradient of $H$, which is zero over a closed curve. So we see that $\omega$ plays again a fundamental role in characterizing the geometry.

Conversely, we can see that each divergenceless field in two dimension admits a stream function $H$ that satisfies \ref{rp-cm-displacementCurl1d}. Geometrically, we can construct $H$ in the following way. Take a reference point $O$ and assign $H(O) = 0$. For any other point $P$, consider the flow of $S$ through any two lines that connect $O$ and $P$ given that the flow through the region contoured by those lines must be zero, the flow through each line must be equal. Therefore the flow through a line that connect $O$ and $P$ only depends on the points, it is path independent. We can assign $H(P) = \int_{OP} \left( S^p dq - S^q dp \right) = \int_{OP} \omega_{ab}  S^b d\xi^a$. We can see that the differentials agree
\begin{equation}
	dH = \partial_q H dq + \partial_p H dp = \partial_a H d \xi^a = S^p dq - S^q dp = \omega_{ab} S^b d\xi^a
\end{equation}
and this will be a valid stream function. Geometrically, at least for the one dimensional degree of freedom, we can understand the difference of the Hamiltonian between two points as the flow of the displacement field between them.
...
\begin{equation}\label{rp-cm-volumeTransformation1d}
	\begin{aligned}
		\hat{\xi}^b &= \hat{\xi}^b(\xi^a) \\
		d\hat{\xi}^b &= \partial_a \hat{\xi}^b d\xi^a \\
		d\hat{\xi}^n &= \left| \partial_a \hat{\xi}^b \right| d\xi^n \\
...
Mathematically, a transformation is volume preserving if the determinant of the Jacobian $\partial_{a} \hat{\xi}^b$ is unitary. If $\hat{q}$ and $\hat{p}$ represent the evolution of $q$ and $p$ after an infinitesimal time step $\delta t$, we have
\begin{equation}
	\begin{aligned}
	\hat{q} &= q + S^q \delta t \\ 
	\hat{p} &= p + S^p \delta t \\ 
	\partial_{a} \hat{\xi}^b &= \left[\begin{array}{ c c }
		1 + \partial_q S^q \delta t & \partial_p S^q \delta t \\
		\partial_q S^p \delta t & 1 + \partial_p S^p \delta t \\
	\end{array} \right] \\
	\left|\partial_{a} \hat{\xi}^b\right| &= (1 + \partial_q S^q \delta t) (1 + \partial_p S^p \delta t) - \partial_p S^q \, \partial_q S^p \, \delta t^2  = 1 + \left(\partial_q S^q + \partial_p S^p \right) \delta t + O(\delta t^2). 
	\end{aligned}
\end{equation}
...
\begin{equation}\label{rp-cm-conservedAreas1d}
	\tag{HM-3}
	\text{Areas are conserved: } \; \left| \partial_a \hat{\xi}^b \right| = 1.
\end{equation}
and \ref{rp-cm-divergenceless1d} are equivalent conditions, and we have a third way to characterize Hamiltonian evolution.

We can reframe the same requirement in a slightly different way. Given two vectors $v^a$ and $w^a$, the area of the parallelogram they form is $v^q w^p - v^p w^q$. This can be rewritten as $v^a \omega_{ab} w^b$, which means we can think of $\omega$ as a tensor that, given two vectors, returns the area of the parallelogram they form. If we denote $\hat{v}^a = \partial_b \hat{\xi}^a \, v^b$ and $\hat{w}^a = \partial_b \hat{\xi}^a \, w^b$ the transformed vectors, the invariance of the area can be written as
\begin{equation}
	v^a \omega_{ab} w^b = \hat{v}^c \omega_{cd} \hat{w}^d.
\end{equation}
Since
\begin{equation}
	\hat{v}^c \omega_{cd} \hat{w}^d = v^a \, \partial_a \hat{\xi}^c \omega_{cd} \partial_b \hat{\xi}^d \, w^b = v^a \, \partial_a \hat{\omega}_{ab} w^b
\end{equation}
the previous equivalence means that $\omega = \hat{\omega}$, that is $\omega$ remains unchanged. In other words, preserving the area for all possible pairs of vector is the same as preserving the tensor $\omega$ that returns the areas. Therefore, once again, $\omega$ plays a very important geometric role.

While condition \ref{rp-cm-conservedAreas1d} is expressed in terms of areas, similar considerations will work for densities because a density is a quantity divided an infinitesimal area. In fact densities
\begin{equation}\label{rp-cm-densityTransformation1d}
	\begin{aligned}
		\left| \partial_{\xi^a} \hat{\xi}^b \right| \hat{\rho}(\hat{\xi}^b) &= \rho(\xi^a).
	\end{aligned}	
\end{equation}
transform in an equal and opposite way. The unitarity of the Jacobian determinant, them, is equivalent to requiring that the density at an initial state is always equal to the density at the corresponding final state. Both areas and densities are transported unchanged by Hamiltonian evolution. Therefore
\begin{equation}\label{rp-cm-conservedDensities1d}
	\tag{HM-4}
	\text{Densities are conserved: } \; \left| \partial_a \hat{\xi}^b \right| = 1.
\end{equation}
...
\begin{equation}
	...
	\end{array} \right]
\end{equation}
is the inverse of $\omega_{ab}$. That is $\omega_{ab} \omega^{bc} = \delta_a^c$. So, again, we see how $\omega$ plays a fundamental geomatrical role.

The four characterizations \ref{rp-cm-displacementCurl1d}, \ref{rp-cm-divergenceless1d}, \ref{rp-cm-conservedAreas1d} and \ref{rp-cm-conservedDensities1d} are all in terms of the mathematical description. While these are useful, the final goal of reverse physics is to find physical assumptions, not just equivalent mathematical definitions. So it is time to step back and try to understand what the math is really about.

Let us first reflect to what we just found out: the defining characteristic of Hamiltonian mechanics is not the transport of points, but the transport of areas and densities. If classical Hamiltonian mechanics were really about and only about point particles, there would be no reason for it to be characterized by \ref{rp-cm-divergenceless1d}, \ref{rp-cm-conservedAreas1d} or \ref{rp-cm-conservedDensities1d}. In fact, there would be no reason for the equations of motions \ref{rp-cm-displacementCurl1d} to be differentiable. Differentiable equations are exactly needed if we need to define the transport of areas, or of densities defined on those areas. Classical point particles, then, are more aptly thought not as points, but as infinitesimal regions of phase space, as distributions so peaked that only the mean value is important.
...
\end{equation}
Note that by reversible here we mean that given the final state we can reconstruct the initial state. Given that areas measure the number of states, \ref{rp-cm-detrev1d} is equivalent to \ref{rp-cm-conservedAreas1d}, which means this is another characterization of Hamiltonian mechanics. We can also see a connection to \ref{rp-cm-conservedDensities1d}. If we assign a density to an initial state, and we claim that all and only the elements that start in that initial state will end in a particular final state, we will expect the density of the corresponding final state to match. That is, if the evolution is deterministic and reversible it may shuffle around a distribution, but it will never be able to spread it or concentrate it.

This makes us understand, at a conceptual level, why a dissipative system, like a particle under linear drag, is not a Hamiltonian system. A dissipative system will have an attractor: a point or a region to which the system will tend to given enough time. This means that, in time, the area around the attractor must shrink, the density will concentrate over the attractor, but this is exactly what Hamiltonian systems cannot do. Therefore Hamiltonian systems cannot have attractors, they cannot be dissipative. The converse is also true: they can't have unstable points or regions from which the system always goes away from.
...
\end{equation}
is equivalent to \ref{rp-cm-conservedAreas1d} and is yet another characterization of Hamiltonian mechanics.
...
\end{equation}
is equivalent to \ref{rp-cm-conservedAreas1d} and another characterization of Hamiltonian mechanics.
