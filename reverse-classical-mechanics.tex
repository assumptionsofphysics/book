...
\subsection{Reversing differentiability}
In the previous section we have finished reversing the top level structures of classical mechanics. While we have seen that phase space is characterized by the form $\omega_{ab}$, which we saw defines the count of state in a coordinate invariant way, mathematically this is not a primitive structure. In fact, to be able to even define a form we need a differentiable manifold, which is something most physicists do not exactly know what it is. Yet, we cannot say we have found all the physical requirements implicit in classical mechanics until we have analyzed all the mathematical structures required by the theory.

Mathematically, a differentiable manifold is a space for which all change of variables are differentiable. That is, whenever we perform coordinate transformations, the Jacobian exists and its determinant is not zero. To allow for state count, density and entropy do be observer independent, we saw that we needed the Jacobian to be unitary. Clearly, before requiring the Jacobian to be unitary, we need to require that it exists, which means that requiring the ability to define state count, density and entropy we need a differentiable manifold to begin with.

To understand this better, both mathematically and physically, consider simply the structure of a manifold. This is a space for which the neighbourhood of every point can be charted by continuous quantities. In general, any invertible coordinate transformation is allowed on a manifold, even those that are not differentiable. This would mean that even if we defined a density on a set of variables $A$, this won't be well defined for another set $B$. On the other hand, we could have another density defined on $B$, which we could not express in terms of $A$. If we have a third set of variables $C$ that is differentiable with respect to $A$, then we can express the first density in terms of $C$ but not the second. In other words, groups of variables that are differentiable to each other ``see'' the same densities within same group, but not across groups. A differentiable structure on a manifold is essentially picking one of these groups. We need to make this selection to be able to define densities in a way that can be expressed in different variables. Furthermore, the phase space symplectic structure further identifies another set of special variables, the canonical ones. These are the differentiable variables that express the state count, the density and the entropy in the correct units.

This give us a sort of hierarchy of variables. We have variables, which just allow us to identifies each point, each case; we have differentiable variables, which additionally allow us to define densities at each point; we have conjugate variables, which are differentiable variables that allow us to define densities and count of states in an invariant way. Mathematically, this correspond at the difference between a manifold, a differentiable manifold and a symplectic manifold.

\subsection{Particle as infinitesimal parts}
Now that we have some understanding of the mathematical structures used, we need to understand what are the physical requirements that they need to capture. Since these requirements ultimately stems from what it is that we are going to describe, we will need to clarify what a particle actually is.

As we saw, thinking of a particle as a finite mass all concentrated at a point does not work. Classical mechanics is not mapping points to points, but densities to densities, phase space areas to phase space areas. But the idea of a point particle fails not just in classical mechanics, it fails in all of physics. In general relativity, a finite mass at a point would puncture spacetime and would not follow a geodesics. In quantum mechanics, a particle perfectly concentrated at a point would have a uniform spread in momentum, requiring infinite kinetic energy. As we already discussed, particles should be understood as infinitesimal regions of phase space, meaning that they can't be a standalone physical object: as they are infinitesimals, they should really be through as the limit of some recursive subdivision.

If we start thinking of a macroscopic object, then, it is more reasonable to model it, not as a point in phase space, but as a distribution $\rho(q^i, p_i)$ over phase space. A macroscopic object, in fact, has a finite extension in space and a finite extension in momentum as well. The distribution, then, tells us exactly how much of the system is at each range of position and at each range of momentum. From the distribution we will also be able to calculate the position and momentum of the center of mass, and if the spread in position and momentum is negligible for the problem at hand, it can be disregarded. That is, more properly, what we should refer to the point particle approximation.

Now, recall the discussion about differential topology: we do not measure densities, we do not measure differential forms, but we measure finite quantities over finite regions. For example, we are going to be able to say that half of the system is in a particular region, or that a tenth of the system is another, smaller, region, and so on. The particles, then, are the limit of this infinitesimal subdivision. If the state of the particle is identified by real valued quantities, then the state space of the particles naturally must be a manifold (i.e. states are identified by real valued quantities), in fact a differentiable (i.e. we must define densities over the infinitesimal subdivision) and symplectic (i.e. states, and therefore densities associated to states, must be observer independent).

\subsection{Divisible vs reducible}

We have seen that particles should be thought as the limit of a process of subdivision, as it is both physically more precise and it leads to the correct math. That is, we have an object, we divide into parts, those parts into other parts, and so on. However, divisibility is not quite the right concept. We need something slightly different, called reducibility, which we introduce with the following example.
...
The fact that $f(U)$ is real valued is also implied by Assumption \ref{assum_infirr} since the parts are infinitesimal and therefore there can't be a smallest increment.\footnote{Technically, this only limits the output of $f(U)$ to be a bounded dense linear order. Rational numbers from zero to one would also satisfy the requirement. To characterize real values, we would need to show that the order is complete and has a countable dense subset. We will leave this details to the Physical Mathematics section.} It also implies, however, that the fraction $f(\{x\})$ associated to a single particle state $x \in S_P$ cannot be a finite number. If that is the case, then the system cannot be further reduced into smaller parts since the single particle state cannot be further subdivided. But this would mean that $x$ is not the state of an infinitesimal part of the system, as a finite part of the system can be found in that state, which is a contradiction. Therefore we must have $f(\{x\}) = 0$ for any single particle state. This means that the particle state space $S_P$ of an infinitesimally reducible system must be charted by continuous variables.\footnote{Again, technically we only found that the space must be, in some sense, dense.}

So far, we have found that, under assumption \ref{assum_infirr}, the state space $S_C$ of a system is the space of all possible functions $f(U) \in [0,1]$ where $U \subseteq S_P$ is a subset of the all possible particle states $S_P$. Furthermore, the particle state space $S_P$ must be charted by continuous variables, it must be a manifold. We now need to recover the notion of differentiability and of density.

To be able to fully characterize the state of the whole system into the state of the parts we need to be able to quantify how many states are there in each region of $S_P$. This means that we will have another set function $\mu(U)$, which will also be countably additive as disjoint regions of space will identify entirely different states, making $\mu$ another measure. However, $\mu$ will be bounded only from below (i.e. no set can have fewer than zero states) as $S_P$ can have potentially infinitely many states. Note that, because of the infinitesimal reducibility assumption, we will be able to find smaller and smaller subsets of $S_P$. Meaning, the value of $\mu$ will keep decreasing and, in the limit of infinitesimal subdivision, we must that that $\mu({x})=0$. That is, a single particle state counts as zero in terms of number of states.

This already tells us that the assumptions of infinitesimal reducibility shouldn't be taken literally. It is clearly a simplyfying assumption and should be understood as such. The fact that an individual particle state has measure zero in terms of the count of states is really telling us that these states are idealization, and do not exist in practice. This, again, reiterate the idea that point particles should not be taken literally as they are idealization.

We have seen that both the measure $f$ for the fraction of the system and the measure $\mu$ for the count of particle states are zero for sets with single points. More in general, we must have that whenever $\mu(U) = 0$, then $f(U)=0$. That is, we cannot assign a finite fraction to a set of state that has no finite state count. That would, again, mean that we have a finite fraction to an infinitesimal subdivision, which is untenable. Mathematically, this means that $f$ is absolutely continuous with respect to $\mu$, and therefore we can define a density $\rho$ such that $f(U) = \int_U \rho d\mu$. That is, the state of the whole system can be described by density over the particle states.

As we said, $S_p$ is a manifold. The measure $\mu$ is a feature of that manifold, in the sense that it depends only on the properties of the particle states, and it will be the same for all states of the whole system. Of all possible state variables that we can use to chart the manifold, then, it is convenient to choose those that $\mu$ as a density of states over units of the state variables. Mathematically, we are asking that the Lebesgue measure induced by the variable $\eta : S_p \to \mathbb{R}$ is absolutely continuous with respect to $\mu$. This will also require that the variables are differentiable to each other: it will lead to the differentiable structure. That is, the differentiable structure of $S_P$ is exactly the ability to express state count and fractions as densities over particle states.

The last requirement comes from the fact that both $f$ and $\mu$ do not depend on the choice of state variables. Therefore the proper expression of $\rho = \frac{df}{d\mu}$ must also not depend on the choice of state variables. This, plus assumption \ref{assum_indep}, requires the existence of the form $\omega$, and recovers phase space, the symplectic manifold.
\begin{equation}\tag{IR-1}
	\parbox{5in}{The state of the whole system is a distribution over phase space (i.e. a symplectic manifold)}
\end{equation}

The very notion of classical states, then, rests on assumption \ref{assum_infirr}, which can be understood as the constitutive assumption of classical mechanics. All classical systems, in fact, do follow the assumption, not just mechanical systems. Continuous mechanics and fluid dynamics assume that the materials are a continuum of infinitesimally small parts; classical electromagnetism assume that the electromagnetic radiation can be decomposed into arbitrarily small signals at all frequencies. Moreover, the assumption clearly fail in quantum mechanics. In that case, we cannot talk about the state of parts of an electron; materials are not a continuum of infinitesimally small parts, the intensity of electromagnetic radiation cannot be made arbitrarily small.
