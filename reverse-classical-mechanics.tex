...
\begin{equation}\label{rp-cm-symplecticNd}
	\tag{SF-1}
	\omega_{ab} = \left[\begin{array}{cc}
...
However, these conditions are not equivalent to \ref{rp-cm-hmd-condEquations}. The displacement field $S^a$ has $2n$ components and is therefore specified by $2n$ functions. The above conditions specify one constraint, and the choice of Hamiltonian uses another choice, leaving $2n - 2$ choices undetermined. In the single degree of freedom case, $n=1$ and no choices are left, which makes everything work.
...
\end{equation}
On the left side we have the generalization of the curl of $S_a$ for a space of arbitrary dimension.\footnote{Those familiar with relativistic electromagnetism will recognize the expression.} It employs the same anti-symmetrization between derivative and field components, but it has a component for each planar direction.\footnote{In three dimension, to each planar direction corresponds one perpendicular linear direction. This fails in all other dimensions, which is why vector calculus is not as useful in the general case. The generalization of the curl is the exterior derivative applied to one forms.} The above equation, then, tells us that $\omega$ is invariant in time if and only if the curl of $S_a$, the rotated displacement field, is zero.
...
Therefore we found that condition
\begin{equation}\label{rp-cm-hmd-condConservedSymplectic}
	\tag{HMMD-2}
	\text{The evolution leave $\omega$ invariant}
\end{equation}
and
\begin{equation}\label{rp-cm-hmd-condCurlRotatedDisplacement}
	\tag{HMMD-3}
	\text{The rotated displacement field $\omega_{ab}S^b$ is curl-free (i.e. null exterior derivative)}
\end{equation}
are equivalent to Hamilton's equations \ref{rp-cm-hmd-condEquations}

This reshuffles a bit the cards from the single degree of freedom. In the that case, we argued that it was the divergenceless of the displacement field that allowed us to find $S^a$ was the (2-d analogue) of the curl of the Hamiltonian. But if we look closer at \ref{rp-cm-hsd-nullGradient}, even in that case we found that the rotated displacement field $\omega_{ab} S^b$ was curl free. And that is the condition that generalizes.

The situation is therefore the following. Conditions \ref{rp-cm-hmd-condEquations}, \ref{rp-cm-hmd-condConservedSymplectic} and \ref{rp-cm-hmd-condCurlRotatedDisplacement} are all equivalent, so they represent full Hamiltonian mechanics for multiple degrees of freedom these imply conditions  \ref{rp-cm-hsd-condDivergenceDisplacement}, \ref{rp-cm-hsd-condConservedVolume} and \ref{rp-cm-hsd-condConservedDensity}