...
\begin{equation}
	\int \left( S^p dq - S^q dp \right) = \int S^a \omega_{ab} d\xi^b = \int S_b d\xi^b
\end{equation}
...
The issue here is that the case of independent variable is maximal as it posits that all combinations of configurations are possible. Therefore, the only way to make variables no longer independent is to decrease the number of distinct cases, which cannot happen during deterministic and reversible evolution. The same must happen in the infinite or the continuous case, because finite ranges must still be comparable with finite sizes which must hold the same property. Therefore when we mix \ref{assum_indep} and \ref{assum_detrev}, the physics tells us that independence of degrees of freedom must be preserved, which means preserving $\omega_{ab}$ as well. 

This is, again, another example of why just looking at the math is not enough. Two physical conditions taken separately may each impose one mathematical condition, but it not true that imposing them together will only impose the conjunction of the two mathematical conditions. One can argue that this is not a problem with math in general, but it is an indication that the math used is not the correct one as it does not seem to be capturing the entirety of the physical conditions.

Note that, in principle, we could ask for the preservation of the independence of d.o.f. without requiring \ref{assum_detrev}. This would mean that $\omega_{ab}$ could change by a single scalar factor, shrinking or stretching each d.o.f. by the same amount. If we understand the volume as entropy, this tells us that the only way we can add entropy into a system while preserving the independence of the degrees of freedom is by dividing that entropy contribution equally among each d.o.f..