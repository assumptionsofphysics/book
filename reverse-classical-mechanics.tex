...
\begin{equation}
	...
	\end{aligned}
\end{equation}
What we can see is that, in each expression, we are taking the derivative with respect to each index. TODO explain the pattern. The pattern generalizes in the following way
\begin{equation}
	\begin{aligned}
		...
		&= \sum_{j=0}^{j=k} (-1)^{j\cdot k} \partial_{i_{j \bmod k+1}} \omega_{i_{j+1 \bmod k} i_{j+2 \bmod k+1} \cdots i_{j+k \bmod k+1}}.
	\end{aligned}
\end{equation}
