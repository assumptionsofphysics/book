...
Now, the motion of a particle under linear drag would seem reversible, in the sense that we are able to, given the final position and momentum, reconstruct the initial values. Mathematically, it maps points one-to-one and would seem to satisfy \ref{rp-cm-detrev1d}, even though it is not a Hamiltonian system. This is a perfect example of how focusing on just the points leads to the wrong physical intuition. Physically, we would say that a one meter range of position allows for more configurations than a one centimeter range, even though technically they have the same number of points. If we understand that states are infinitesimal areas of phase space, we can see that a dissipative systems, though it does map the center points of infinitesimal areas one-to-one, it does not map the full infinitesimal area one-to-one.\footnote{Mathematically, this argument is not precise but this is not how we evaluate physical arguments: we evaluate them by checking whether they lead to the right intuition and whether they agree with others. Taking these intuitions and making them mathematically precise will be preview of physical mathematics.} In this sense dissipative systems fail to be reversible.

TODO: argument on first law of thermodynamics?

The idea that a dissipative system is not reversible sounds true on thermodynamic grounds. However, we should be clear that this is a different notion: thermodynamic reversibility is not the ability to reconstruct the initial state, but rather the existence of a process that can undo the change. Alternatively, a process is  thermodynamically reversible if it conserves thermodynamic entropy, which is a more precise characterization.\footnote{The actual existence of a reverse process is not something that can always be guaranteed.} The fundamental assumptions of statistical mechanics tells us that the thermodynamic entropy $S = \log W$ is the logarithm of the count of states, which corresponds to volume in phase space. Conservation of areas of phase space, then, is equivalent to conservation of entropy. Therefore
...
\begin{equation}
	...
	\end{array} \right].
\end{equation}
The determinant gives us a coordinate independent quantity to characterize the uncertainty. If the distribution is narrow enough, we can use a linear transformation to see how the uncertainty evolves after an infinitesimal time step $\delta t$. We have
\begin{equation}
...
We have found seven equivalent characterizations that link Hamiltonian mechanics, vector calculus, differential geometry, statistical mechanics, thermodynamics, information theory and plain statistics. Though we only talked about the case of a single degree of freedom, it gives us a much better idea of what systems Hamiltonian mechanics is supposed to describe, those that undergo deterministic and reversible evolution. We can see how this concept is implemented mathematically: it is not simply a one-to-one map between points, but between infinitesimal regions. This means that areas, thermodynamic entropy and information entropy are conserved. Looking at it differently, the map between past and future has to be done at the same level of uncertainty. But the most important lesson is that the foundations of classical mechanics are not disconnected from the foundations of all other disciplines we connected. Therefore we will have to understand as much as possible those connections as well.

For example, we have shown that determinism and reversibility implies the existence and conservation of energy in the form of the Hamiltonian. This may sound a bit surprising, but we can actually make a reasonable argument for it with idea that are already established in physics. If a system is deterministic and reversible it means it's evolution depends only on the state of the system, and therefore does not depend on anything else. A system whose evolution does not depend on anything else is isolated, and from thermodynamic we know that an isolated system conserves energy. Therefore what we have found is simply a stronger version of the law of conservation of energy.