...
Mathematically, a transformation is volume preserving if the determinant of the Jacobian $\partial_b \hat{\xi}^a$ is unitary. If $\hat{q}$ and $\hat{p}$ represent the evolution of $q$ and $p$ after an infinitesimal time step $\delta t$, we have
\begin{equation}
	\begin{aligned}
	\hat{q} &= q + S^q \delta t \\ 
	\hat{p} &= p + S^p \delta t \\ 
...
\begin{equation}\label{rp-cm-conservedAreas1d}
	\tag{HM-3}
	\text{Areas are conserved: } \; \left| \partial_b \hat{\xi}^a \right| = 1.
\end{equation}
...
\begin{equation}\label{rp-cm-densityTransformation1d}
	...
	\end{aligned}	
\end{equation}
transform in an equal and opposite way with respect to areas (i.e. the Jacobian determinant is on the other side of the equality). The unitarity of the Jacobian determinant, them, is equivalent to requiring that the density at an initial state is always equal to the density at the corresponding final state. Both areas and densities are transported unchanged by Hamiltonian evolution. Therefore
\begin{equation}\label{rp-cm-conservedDensities1d}
	\tag{HM-4}
	\text{Densities are conserved: } \; \left| \partial_b \hat{\xi}^a \right| = 1.
\end{equation}
