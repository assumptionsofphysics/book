...
\begin{align}
	...
	&\parbox{5in}{The symplectic form $\omega_{ab}$ can be expressed in kinematic variables, and its components are a function of position only. (TODO: not true)}
\end{align}
...
Before we go too far, let's see if this reasoning extends to the general case. This time we want to preserve the area on all independent degrees of freedom, that is we want to preserve $\omega_{ab}$. We want to write the transformed $\hat{\omega}_{ab}$ and set it equal to the original one. We have
\begin{equation}
	\begin{aligned}
...
The constraints coming for the off diagonal terms are the same, and they are
\begin{equation}
	\begin{aligned}
...
We found that, in phase, we can indeed take arbitrary transformations of the position variables and, by asking the form $\omega_{ab}$ to be invariant, we recover the transformation law of momentum. Each component of momentum has to be expressed in inverse units of the respective position components. This means that uniform distribution over phase space are mapped to uniform distributions, the density at each point is coordinate invariant and the entropy is the same for all observers. We reach the following
\begin{insight}
	The structure of phase space makes the principle of relativity and the principle of indifference compatible.
\end{insight}

The invariance of $\omega_{ab}$ under change of spatial coordinates means 