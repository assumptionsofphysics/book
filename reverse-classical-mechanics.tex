...
\begin{equation}
	\mathcal{H} = \frac{1}{2m} (-\sqrt{c^2 |p_i|^2 + (mc^2)^2 } - E) (\sqrt{c^2 |p_i|^2 + (mc^2)^2 } - E)
\end{equation}
Given that the solutions for energy have opposite sign, one finds
\begin{equation}
	\lambda = \frac{1}{2m}(-E -E) = \frac{E}{m}
\end{equation}
...
The minus sign for the energy, then, has a geometrical physical meaning. If we used $E$ instead of $-E$ as the variable, the minus sign would show up in the form $\omega_{ab}$, which would probably be better as it would make the geometrical feature more clear. However, $p_\alpha = \{ E, p_i\}$ would not match the definition used in relativity, so it would be confusing in a different way. If there is a better overall notation and grouping, we haven't yet found it.

We have reached an interesting result: just by introducing generalized coordinate transformations in Hamiltonian mechanics we have already recovered features of both special relativity and quantum field theory.
