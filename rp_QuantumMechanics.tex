...
Let's start restating and analyzing the following condition:
\begin{equation}\label{rp-qm-qp-condStatesAreRays}
	\tag{QP-RAY}
	\eqtext{The state of a quantum system is represented by a ray $c | \psi\>$ of a complex inner product space $\mathcal{H}$.}
\end{equation}
As the state is represented by a ray, a change in norm or phase does not change the physics. Since all the physics is given by the Born rule
\begin{equation}
	p(\phi|\psi) = \frac{\< \phi | \psi \>\< \psi | \phi \>}{\< \psi | \psi \>\< \phi | \phi \>},
\end{equation}
we can simply verify the Born rule is invariant if we change $\psi$ to $\rho e^{\imath \theta} \psi$:
\begin{equation}
...
A quantum state, then, is not represented by a single vector, but an equivalence class of vectors that give the same results under the Born rule. On the continuum, with wavefunctions $\psi(x)$, there is an additional degeneracy. The inner production between two wave functions is given by:
\begin{equation}
	\< \phi | \psi \> = \int_X \phi^*(x) \psi(x) dx.
\end{equation}
Note that the integral does not vary under a change of $\phi(x)$ over over a set of measure zero. For example, if we changed $\psi(x)$ such had it returned zero over the rationals, the inner product wouldn't change, therefore the physical state does not change either. Mathematically, when we write the vector $|\psi \>$ we are actually talking about the set of all functions that are equivalent to the wave function $\psi(x)$. This is not something peculiar to quantum mechanics, as it this happens in classical probability as well. If we have a probability density $\rho(x,p)$, a change over a measure zero set will not change the expectation of any variable. To recap, a quantum state is represented by a ray, which is an equivalence class of vectors, which is an equivalence class of wavefunctions.
