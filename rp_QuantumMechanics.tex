...
Over BT
\begin{itemize}
	\item[?] State space and topology is not enough to identify classical vs quantum
\end{itemize}
...
The following are established over QST
\begin{itemize}
	\item[?] State space and topology is not enough to identify classical vs quantum
...
Alternatively
\begin{itemize}
	\item[QPT] The pure states are rays in a Hilbert space
	\item[BR] Born rule in terms of probability of something?
	\item[QST] Ensemble space is QPT + BR
\end{itemize}
...
Over QST+BR+PM $\implies$ DR-PSEQ and DR-MSEQ, and QST+BR+DR $\implies$ PM-DEQ
\begin{itemize}
...
For an explicit example where the order of basis matters consider the state
\begin{equation}
...
However, for a pure state we can consider the domain of the operator as an indicator of what states have well defined expectation values. Let's check if $\psi$ could possibly be in the domain of $\mathcal{O}$. For that, calculate
\begin{equation}
	\begin{split}
