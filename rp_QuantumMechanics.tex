...
Over BT
\begin{itemize}
	\item[?] QP is not enough to identify classical vs quantum
	\item[?] QE $\implies$ QP 
	\item[?] QE is enough to rule out classical, but not to fully identify quantum
	\item[?] QE $+$ BR $=$ QST
\end{itemize}
...
\begin{itemize}
	...
	\item[QE-ENS] Ensembles are density operators (recover pure states as extreme points, recover ensembles as mixtures)
	\item[QP-EXTR] A pure state is a density operator with only one non-zero eigenvector (EXTR for extreme point - It was SUBS, what was SUBS?)
	\item[QE $\implies$ QP] Show that pure states are ensembles with only one non-zero eigenvector (i.e. they are extreme points)
	\item[QP-PROJ] A pure state is a projector
...
\begin{itemize}
	\item[BR-ME] Orthogonal states are mutually exclusive
	\item[BR-PROB] The probability of measuring a final state given an initial state is well-defined.
	\item[BR-ANG] The born rule returns the cosine(?) of the angles between pure
	\item[BR-ENT] The entropy is the von Neumann entropy
\end{itemize}
...
Random things
\begin{itemize}
	\item tensor product/composite systems
...
Insight: Non-Hermitian evolution is unphysical
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\item Different linearities
			\item Gap between linearities
			\item Basis and gauge choice
		\end{enumerate}
		\item Born rule
		\begin{enumerate}
