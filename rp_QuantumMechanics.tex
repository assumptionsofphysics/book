...
\begin{condition}[Quantum states]{QST}
	The state space is given by the rays of a complex inner product space $\mathcal{H}$ associated to that system. An ensemble is represented by a density operator $\rho : \mathcal{H} \to \mathcal{H}$, that is a positive semi-definite trace one self-adjoint operator. The entropy is given by the von Neumann entropy.
\end{condition}
