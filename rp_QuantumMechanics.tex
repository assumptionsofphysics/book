...
\subsection{Ambiguous trace}\label{rp_qm_trace}
For an explicit example where the order of basis matters consider the state
\begin{equation}
	\rho = \sum_{n=1}^{\infty} p_{n} | \psi_{n} \rangle \langle \psi_{n} | = \frac{ 6 }{ \pi^{2} } \sum_{n=1}^{\infty} \frac{ 1 }{ n^{2} }
\end{equation}
and the observable
\begin{equation}
...
TODO: add the equivalent example with a pure state and show that it can't be in the domain of the unbounded operator.
