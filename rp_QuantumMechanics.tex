...
In this section we are going to summarize the standard Hilbert space formulation of quantum mechanics. The goal is to have a general understanding of the math and how it naively maps to the physics. We will also define the notation that we are going to use throughout this chapter.
...
The postulates of quantum mechanics just give us the mathematical framework, without stating exactly this type of relationships. We know, for example, that the physics is in the rays and the Born rule. What part, then, of the Hilbert space is redundant, like the choice of gauge for a potential, and which part is unphysical, like the extension from real to complex valued charge distributions? Do all rays correspond to physically meaningful states? Since linearity is defined on the vectors, and the vectors themselves are not the physical object, does the linearity of the vector space correspond to a physical requirement? These are the questions we will answer in this section.

\subsection{Vector vs projective space}

In order to make a thorough analysis, a common strategy in Reverse Physics is to break the mathematical structure into the smallest structures required for the physical definition. The most basic item in quantum mechanics is the notion of state, which is defined as a ray in a Hilbert space. However, the definition of rays and projective space do not require the inner product: just the vector space structure. The vector space needs to be complex, as the projective space will be different depending on the scalar field. Therefore, let us start with the following basic condition.

\begin{condition}[Quantum states are rays]{QP-RAY}
	The state space of the system is represented by the projective space given by the rays of a complex vector space $\mathcal{H}$.
\end{condition}

We should verify that physics only depends on the ray, and, since the ray is an equivalence class, not the choice of representative. Since all the physics is given by the Born rule
\begin{equation}
	p(\phi|\psi) = \frac{\< \phi | \psi \>\< \psi | \phi \>}{\< \psi | \psi \>\< \phi | \phi \>},
\end{equation}
we can verify that the Born rule is indeed invariant under the change of representative $|\psi\>$ to $c |\psi\>$ with $c \in \mathbb{C} \setminus \{0\}$:
\begin{equation}
...
Let's verify all of this in the math. First, the map is collinear, meaning that it maps a ray to another ray. That is:
\begin{equation}
	\begin{aligned}
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
This is a linear map, and therefore it is also colinear: maps rays to rays. It will also map projectors to projectors and therefore will preserve \ref{rp-qm-qp-condStatesAreRays} and all its equivalent conditions.
