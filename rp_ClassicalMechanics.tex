...
This, in retrospect, matches how classical mechanics is used in practice: planets, cannonballs, pendulums, beads on a wire, all the objects we study with classical mechanics are not point-like objects. They can be considered point-like if their size is negligible compared to the scale of the problem. If the distance between two celestial bodies is smaller than the sum of their radius, the point particle approximation clearly fails. This is also consistent with fluid dynamics and continuum mechanics, where we are literally studying the motion of infinitesimal parts of a material. It is interesting to see echoes of these considerations present in the mathematics.\footnote{We will want to investigate this link in more detail later.}
...
\begin{equation}\label{rp-cm-emHamiltonianConstraintDerivation}
	\begin{aligned}
...
\begin{equation}\label{rp-cm-emTensorAndInertialPseudotensor}
	\begin{aligned}
...
\section{Directional degree of freedom}\label{rp-cm-secDirectionDOF}
...
\openproblem{Extend the classical uncertainty principle }{rp-cm-extendClassicalUncertainty}{Classical mechanics}{Hamiltonian mechanics, Information theory, Symplectic geometry, Statistics}{https://www.youtube.com/watch/8c0v_1j9bYg}
{Currently, the classical uncertainty principle is fully worked out for canonical variables of a single DOF. We would like to generalize the expression for non-canonical variables, relating it to the Poisson brackets, and to multiple DOF.}

To extend to non-canonical variables, we may use a generalized (non-linear non-canonical) coordinate change and study how the entropy/uncertainty changes. These changes will be related to the Jacobian which, for a single DOF, is connected to the Poisson bracket. Therefore it should be possible to formulate the minimum uncertainty relationship based on the Poisson brackets between variables (likely, the expectation of the Poisson bracket).

To extend to the multiple DOF case, we essentially want to study how the covariance matrix (i.e. a symmetric matrix/rank two tensor) over phase space (i.e. a symplectic manifold) under all possible linear Hamiltonian evolutions (i.e. the symplectic group). That is, under the symplectic group.

In symplectic geometry, it is standard to use the block matrices given by the conjugate quantities. That is:
\begin{equation}
	\Sigma = \left[\begin{array}{cc}
		\Sigma_{q^i q^j} & \Sigma_{q^i p_j} \\
		\Sigma_{p_i q^j} & \Sigma_{p_i p_j} 
	\end{array} \right]
\end{equation}
But this does not help us understand how the uncertainty can move from one DOF to the other. We would like to study the following decomposition:
\begin{equation}
	\Sigma = \left[\begin{array}{cccc}
		\Sigma_{(q^1 p_1)(q^1 p_1)} & \Sigma_{(q^1 p_1)(q^2 p_2)} & \cdots & \Sigma_{(q^1 p_1)(q^n p_n)} \\
		\Sigma_{(q^2 p_2)(q^1 p_1)} & \Sigma_{(q^2 p_2)(q^2 p_2)} & \cdots & \Sigma_{(q^2 p_2)(q^n p_n)} \\
		\vdots & \vdots & \ddots & \vdots \\
		\Sigma_{(q^n p_n)(q^1 p_1)} & \Sigma_{(q^n p_n)(q^2 p_2)} & \cdots & \Sigma_{(q^n p_n)(q^n p_n)}
	\end{array} \right]
\end{equation}
The diagonal blocks represent the uncertainty within a DOF. The off-diagonal blocks represent correlations between DOFs. What we want to understand how the uncertainty travels during Hamiltonian evolution.

For example, if we start with a packet with no correlation across multiple degrees of freedom and equal uncertainty within each d.o.f. (a condition related to thermodynamic equilibrium), do we find that the uncertainty in each d.o.f. cannot become lower than the initial one?
...
The idea is to explore more in general the relationship between information theory, Hamiltonian mechanics and thermodynamics. The conjecture is that Hamiltonian dynamics preserves, at least locally, the entropic relationships that one would find at equilibrium. The classical uncertainty relationship is essentially an interplay between the total uncertainty (which is conserved) and the ones along the individual variables (which can be increased by introducing correlations between them).

\openproblem{Find a geometric characterization for $G_{\alpha\beta\gamma}$}{rp-cm-geometricInterpretationOfG}{Classical mechanics}{Differential geometry, Riemmanian geometry, Symplectic geometry, General relativity}{https://www.youtube.com/watch/cXtFbUfn-Ws}
{In \ref{rp-cm-emTensorAndInertialPseudotensor} we have found that the pseudo-tensor $G_{\alpha\beta\gamma} = \partial_{x^\alpha} g_{\beta \gamma} - \partial_{x^\beta} g_{\alpha \gamma}$ plays a fundamental role in connecting the symplectic form (and therefore the geometry/entropy of classical mechanics) to the geometry of space-time. We still lack a good geometric and physical understanding of that object.}

As we saw before, the expression appears with the EM tensor in the spatial-temporal part of the symplectic form when it is expressed in terms of position and velocity. It also appears as a “correction” for the raised anti-symmetrized covariant derivative. This means the expression should have both geometrical and physical significance, which merits further study. In Cartesian coordinates, or in general in any coordinate system for which the metric tensor is constant, $G_{\alpha\beta\gamma}$ is zero. Conversely, we can imagine non-inertial coordinate systems on flat space for which $G_{\alpha\beta\gamma}$ is not zero.

What we would like to understand, then, are things like:
\begin{itemize}
	\item What does $G_{\alpha\beta\gamma}$ represent geometrically?
	\item Is it related to non-inertiality of frames and therefore the presence of inertial forces?
	\item Can we characterize coordinate systems for which $G_{\alpha\beta\gamma}=0$? Can we always have such coordinate systems regardless of curvature for at least a neighbourhood of each point?
\end{itemize}

As for characterizing $G_{\alpha\beta\gamma}=0$, note that if $\partial_{\alpha} g_{\beta \gamma} - \partial_{\beta} g_{\alpha \gamma}$, we are saying that, for each fixed $\gamma$, the curl is zero. Then, at least locally, $g_{\alpha \beta} = \partial_\alpha h_\beta$ where $h_\beta$ is a covector of vector potentials. Since $g_{\alpha \beta}$ is symmetric, $\partial_\alpha h_\beta = \partial_\beta h_\alpha$. Which means $\partial_\alpha h_\beta - \partial_\beta h_\alpha = 0$. This, again, is a zero curl and therefore $h_\alpha = \partial_\alpha S$. Therefore, $g_{\alpha\beta} = \partial_\alpha \partial_\beta S$ the metric is the Hessian of a scalar function. Does this link to Hessian geometry in a way that is physically interesting?

\openproblem{How much of general relativity is already in particle dynamics}{rp-cm-linkBetweenEmAndG}{Classical mechanics}{Differential geometry, Riemmanian geometry, Symplectic geometry, General relativity}{}
{In \ref{rp-cm-emHamiltonianConstraintDerivation} we have derived the equation for massive particles under gravitational and electromagnetic fields. It still needs to be understood whether the two fields can be chosen independently or they must satisfy some relationships, which may lead to a hint of general relativity in particle mechanics.}

The general idea is that the Hamiltonian constraint is a constant for all states and it is also the generator of the affine parameter. The question is whether this poses a constraint across $g_{\alpha\beta}$ and $A_\alpha$. If there is, it would mean that there are some hints of general relativity in plain particle dynamics. If there isn’t, then the link between curvature and energy is a separate assumption from those required to derive particle dynamics under scalar and potential forces. It is likely that there isn't.

\openproblem{Special relativity as a theory of integration}{rp-cm-relativityAsIntegraion}{Special Relativity}{Classical mechanics, Riemannian geometry, Measure theory}{}
{See how much of special relativity can be recovered simply by positing normalization of probability over space across coordinate transformations.}

The idea is that while space and time variables have the same role when identifying an event (i.e. they are all coordinates), they play different roles under integration. If we have a probability distribution, for example,
...
for all surfaces $U$ at equal time. A boosted observer will see time and space mix, but the constraint will have to apply at equal time of the boosted observer. Space-like surfaces, then, would be identifiable as those surfaces over which the probability constraint is satisfied. Therefore, a coordinate transformation cannot mix space-like with non-space-like surfaces.
...
If we take a spatial direction as a degree of freedom, as we had seen in section \ref{rp-cm-secDirectionDOF}, phase space is the two-sphere (the only sphere that is also a symplectic manifold) identified by all
unitary vectors of components $S_i$. The conjugate quantities are given by the angle over a plane and the component perpendicular to the plane. For example, we have $\{\theta^{xy}, S_z\}=1$. The Hamiltonian
is given by $H=\mu B^{k}S_k$. The problem is to find the ``correct'' space with the ``correct'' conjugate variables
for the relativistic case.

This problem is connected to the problem of finding a classical analogue of a spin 1/2 system. The state space for a spin 1/2 system, in fact, is just a 2-sphere, the space of directions in space. Picking the state of a spin 1/2 system, then, is exactly like picking a direction in space. The corresponding relativistic space, then, is the space of all possible orientations boosted in all directions. This means keeping track of the vector of spin at rest plus its boost.

At rest, the vector of spin is simply $S_\alpha = [ 0 \; S_x \; S_y \; S_z ]$, which is a space-like vector. Similarly, at rest the velocity is $u^\alpha = [c \; 0 \; 0 \; 0]$ is a time-like vector. The two vectors are therefore orthogonal $S_\alpha u^\alpha = 0$. Suppose we are in a rest frame for which spin is oriented along $z$, then we can write
\begin{align*}
	u^\alpha &= c \frac{1}{2} (n_R^\alpha + n_L^\alpha) \\
	s_\alpha &= \frac{\hbar}{2} \frac{1}{2} (n_R^\alpha - n_L^\alpha)
\end{align*}
where 
\begin{align*}
	n_R^\alpha &= [ 1 \; 0 \; 0 \; 1 ] \\
	n_L^\alpha &= [ 1 \; 0 \; 0 \; -1 ]
\end{align*}
Since the relationship is valid in one frame, it will be valid in all frames. Note that these correspond exactly to the right and left components for a Dirac field, which means that these quantity do fully identify a boosted spinor. What still needs to be understood are the conjugate quantities.

It is not clear whether the correct generalization is a direction in space-time, or a plane of rotation
...
\openproblem{More in depth exploration of coordinate invariant measures/entropy}{rp-cm-invariantMeasures}{Measure theory}{Classical mechanics, Information theory, Complex analysis}{}
{In our derivation of classical mechanics, we recover symplectic spaces by requiring that the measure needed to quantify the number	of states in a region must be coordinate invariant. This is equivalent to having a way to define an invariant entropy. The argument only works in classical mechanics, and, since a symplectic/Poisson structure exists in quantum mechanics as well, we need an argument that works more in general.}

We are essentially looking for an argument that continuous quantities must be paired if we want to have an entropy that is invariant under coordinate transformation. This means, indirectly, to formalize units and definitional dependencies between variables.

The mathematics of it should be related to \href{https://en.wikipedia.org/wiki/Generalized_complex_structure}{generalized complex structures}, as this is a way to understand the similarities between symplectic spaces and complex vector spaces. One way to approach the problem, then, would be to see what properties measures possess on the complex plane, which may be related to why integration on the complex plane is more powerful than on the real line.

Another approach would be in terms of group theory. Coordinate transformations form a group, and the Lie algebra of that group will have to relate to the Lie algebra of symplectic transformations.

\openproblem{Express Liouville theorem for position and velocity}{rp-cm-LiouvilleInVelocity}{Classical mechanics}{Differential geometry, Symplectic geometry}{}
{To better understand the difference between dynamics and kinematics, it would be interesting to express Liouville theorem in terms of position and velocity instead of position and momentum.}

The Liouville theorem  of Hamiltonian mechanics has its simplest expression in terms of conjugate quantities (i.e. position and momentum). In those cases where we can express momentum as a function of position and velocity (which corresponds to the existence of a Lagrangian), one should be able to express the same relationship in terms of position and velocity. Given that $p(x,v)$ will depend on the Hamiltonian, the expression may include
a reference to the Hamiltonian (or the Lagrangian).

The Liouville theorem in physical terms means conservation of the number of states during time evolution. Position and conjugate momentum provide the best variables to keep that count, as the area using those units provides
a direct count of states. However, conjugate momentum is not a directly measured quantity, as opposed to kinematic momentum.
