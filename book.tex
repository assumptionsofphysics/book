...
Logical consistence, though, is not just a property of individual statements. Consider the following two sentences:
\begin{description}
	\item \statement{the next sentence is true}
	\item \statement{the previous sentence is false}
\end{description}
Each assertion, by itself, would be fine but their combination leads to a logical inconsistency. For this reason we group statements into logical contexts, a set of statements that, all together, are non-contradictory and have well defined logical relationships.
...
\begin{mathSection}
	...
	Let $\bar{\stmt} \in \logCtx$ be a statement and $S \subseteq \logCtx$ be a set of statements. We say $\stmt$ \textbf{depends on} $S$ (or that it is a function of $S$) if we can find a function $f_\Bool : \Bool^S \to \Bool$ such that for every possible assignment $a \in \pAss$ we have $a(\bar{\stmt}) = f_\Bool (\{a(\stmt)\}_{\stmt \in S})$.
\end{defn}
...
\end{coro}
\begin{proof}
	Statement equivalence is defined in terms of truth equality within all possible assignment and will inherit reflexivity, symmetry and transitivity from it.
\end{proof}
